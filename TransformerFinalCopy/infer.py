...
def analyze_stance_phase_torques(
    ...
    # Select vector file based on input source and noise settings
    if input_source == "mocap":
        vector_filename = "KneeToCOP_Vectors_Mocap.npy"
    elif use_noised:
        vector_filename = "KneeToCOP_Vectors_noised.npy"
    else:
        vector_filename = "KneeToCOP_Vectors.npy"
        
    kam_base = Path(str(data.get("input_processed_dir") or source_processed_dir(Path(data.get("trial_dir", "")), input_source)))
    kam_path = kam_base / vector_filename
    
    print(f"   🔍 KAM vector source: {vector_filename} (Source: {input_source}, Noised: {use_noised})")
    
    if kam_path.exists():
        knee_cop = np.load(kam_path)
        pred_grf = predictions['grf']
        gt_grf = ground_truth['grf']
        
        # Match lengths
        n_frames = len(pred_norm)
        if len(knee_cop) > n_frames:
            knee_cop = knee_cop[:n_frames]
        elif len(knee_cop) < n_frames:
            pad = n_frames - len(knee_cop)
            knee_cop = np.pad(knee_cop, ((0, pad), (0, 0)), mode='edge')
            
        z_vec_l = knee_cop[:, 5]
        y_vec_l = knee_cop[:, 4]
        
        # Multiply vertical component (Z) by horizontal GRF (Y), subtract horizontal component (Y) by vertical GRF (Z)
        grf_y_pred_l = pred_grf[:, 4]
        grf_z_pred_l = pred_grf[:, 5]
        kam_l_pred = (z_vec_l * grf_y_pred_l) - (y_vec_l * grf_z_pred_l)
        kam_l_pred_norm = kam_l_pred / norm_factor * 100
        
        grf_y_gt_l = gt_grf[:, 4]
        grf_z_gt_l = gt_grf[:, 5]
        kam_l_gt = (z_vec_l * grf_y_gt_l) - (y_vec_l * grf_z_gt_l)
        ...
        try:
            grf_y_pred_r = pred_grf[:, 1]
            grf_z_pred_r = pred_grf[:, 2]
            kam_r_pred = (z_vec_r * grf_y_pred_r) - (y_vec_r * grf_z_pred_r)
            kam_r_pred_norm = kam_r_pred / norm_factor * 100
            
            grf_y_gt_r = gt_grf[:, 1]
            grf_z_gt_r = gt_grf[:, 2]
            kam_r_gt = (z_vec_r * grf_y_gt_r) - (y_vec_r * grf_z_gt_r)
