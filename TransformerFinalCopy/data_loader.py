...
def source_processed_dir(trial_root: Path, source: str = "video") -> Path:
    """Resolve Video/ProcessedData or MoCap/ProcessedData for a trial root."""
    ...
    return trial_root / folder / "ProcessedData"
...
def load_single_trial(
    ...
    try:
        ...
        if input_source_norm == "mocap":
            knee_to_cop_vectors = load_npy_robust("KneeToCOP_Vectors_Mocap.npy", base_dir=load_path)
        elif input_use_noised:
            knee_to_cop_vectors = load_npy_robust("KneeToCOP_Vectors_noised.npy", base_dir=load_path)
        else:
            knee_to_cop_vectors = load_npy_robust("KneeToCOP_Vectors.npy", base_dir=load_path)
    ...
    def __init__(self, trials: List[Dict], window_size: int = 64, stride: int = 16,
        ...
        for trial_info in trials:
            ...
            try:
                ...
                if self.noised_gt and not self.opencap_val:
                    if self.use_grf_norm_cop:
                ...
                if trial_len is None:
                    ...
                    pos_name = _with_file_suffix("pos_inputs.npy") if self.use_noised else "pos_inputs.npy"
