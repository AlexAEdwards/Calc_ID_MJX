...
def discover_all_trials(
    ...
    ``layout="experiment"`` is the same trial layout with the source study as an
    extra top level: Experiment/Subject/Trial_*/ProcessedData.  Discovered trials
    carry an extra ``experiment`` key.
    ...
    if layout not in {"trusted", "experiment", "opencap"}:
        raise ValueError(
            f"Unsupported trial discovery layout '{layout}'. Expected 'trusted', 'experiment', or 'opencap'."
        )
    layout_version = {
        "trusted": "trusted_processed_v1",
        "experiment": "experiment_subject_processed_v1",
        "opencap": "opencap_video_mocap_processed_v1",
    }[layout]
    ...
    try:
        top_level = [s for s in data_dir.iterdir() if s.is_dir() and not s.name.startswith('.')]
    except Exception as e:
        print(f"   ❌ Error accessing data directory: {e}", flush=True)
        return []

    # (subject_path, experiment_name) pairs; experiment is "" for the flat layouts.
    if layout == "experiment":
        # Only real experiment folders, per the layout manifest when present.
        # Taking every top-level directory would sweep in sibling folders of
        # quarantined subjects (e.g. UnwantedSubjects/) that are excluded in the
        # flat layout purely because they sit one level too deep.
        from experiment_groups import list_experiment_dirs  # local import: leaf module

        experiment_paths = list_experiment_dirs(data_dir)
        skipped = sorted({p.name for p in top_level} - {p.name for p in experiment_paths})
        if skipped and not _quiet_loader_logs():
            print(f"   ↷ Skipping non-experiment director(ies): {skipped}", flush=True)
        subject_entries = []
        for experiment_path in experiment_paths:
            try:
                children = sorted(experiment_path.iterdir())
            except Exception:
                continue
            for subject_path in children:
                if subject_path.is_dir() and not subject_path.name.startswith('.'):
                    subject_entries.append((subject_path, experiment_path.name))
    else:
        subject_entries = [(s, "") for s in top_level]

    def process_single_subject(entry):
        subject_path, experiment_name = entry
        subject_trials = []
    ...
    if not _quiet_loader_logs():
        print(f"   ⚙️  Processing {len(subject_entries)} subjects using {subject_scan_workers} threads...")
    with ThreadPoolExecutor(max_workers=subject_scan_workers) as executor:
        futures = {executor.submit(process_single_subject, e): e for e in subject_entries}
...
def compute_normalizers_from_loader(data_loader: TrialDataLoader, max_batches: int = 100,
                                    stance_only: bool = False) -> Dict[str, Normalizer]:
    ...
    Args:
        ...
        stance_only: If True, estimate COP/GRF Z-score stats over in-contact frames
            only (per foot), so swing-phase zeros do not bias the mean/std.

    Returns:
        Dictionary of normalizers for each data type
    """
    print(f"   Sampling up to {max_batches} batches for statistics"
          f"{' (COP/GRF stance-only)' if stance_only else ''}...", flush=True)
    ...
    for batch in data_loader:
        ...
        if stance_only:
            contact_samples.append(np.array(batch["contactBoolean"]))
    ...
    # Optional stance-only stats for COP/GRF: mask out swing (non-contact) frames per
    # foot so the Z-score mean/std reflect the values the model actually predicts
    # (rather than being pulled toward the swing-phase zeros).
    contact_flat = None
    if stance_only and contact_samples:
        contact_flat = np.concatenate([x.reshape(-1, x.shape[-1]) for x in contact_samples], axis=0)

    def _stance_normalizer(flat, col_to_foot, name, eps):
        """Build a Normalizer using only in-contact frames (per foot) for mean/std."""
        if contact_flat is None:
            return Normalizer(flat, eps=eps, name=name)
        # col_to_foot maps each output column to its contact channel (0=right, 1=left).
        foot_mask = contact_flat[:, col_to_foot] > 0.5          # (N, ncol)
        masked = np.where(foot_mask, flat, np.nan)
        with np.errstate(invalid="ignore"):
            mean = np.nanmean(masked, axis=0, keepdims=True)
            std = np.nanstd(masked, axis=0, keepdims=True)
        # Guard against all-NaN columns (a foot never in contact in the sample).
        mean = np.where(np.isfinite(mean), mean, 0.0)
        std = np.where(np.isfinite(std), std, eps)
        n = Normalizer(data=None, name=name)
        n.mean = mean.astype(np.float64)
        n.std = np.where(std < eps, eps, std).astype(np.float64)
        return n
    ...
    normalizers = {
        ...
        "cop": _stance_normalizer(cop_flat, np.array([0, 0, 1, 1]), "cop", OUTPUT_EPS),
        "grf": _stance_normalizer(grf_flat, np.array([0, 0, 0, 1, 1, 1]), "grf", OUTPUT_EPS),
...
# --- Extracted to core/losses.py in Stage 5; re-exported so train.py's own
# --- make_train_step / make_eval_step / plot_predictions / main still resolve them.
from core.losses import (  # noqa: E402,F401
    _normalize_vector,
    _safe_vector_norm,
    compute_predicted_knee_to_cop_vectors,
    compute_total_loss,
    huber_loss,
    mse_loss,
    project_rotation_matrices,
    select_torque_jacobians,
    ...
    def train_step(state, batch, loss_weights, dropout_rng, epoch):
        def loss_fn(params):
            ...
            loss, metrics = compute_total_loss(pred, batch, normalizers, loss_weights,
                                             ...
                                             compute_effect_diagnostics=compute_effect_diagnostics,
                                             robust_loss=robust_loss,
                                             huber_delta=huber_delta,
                                             contact_mask_source=contact_mask_source,
                                             contact_mix_max_alpha=contact_mix_max_alpha,
                                             use_full_id_gt_for_torque=use_full_id_gt_for_torque)
...
def make_eval_step(normalizers: Dict, use_contact_weighting: bool, magOnOff: bool, contactOnOff: bool, only_supervise_stance: bool, contact_weight_multiplier: float, magWeight: float, total_epochs: int, dof_weights_dict: Dict = None, cop_mask: bool = True, use_grf_norm_cop: bool = False, use_gt_jacob_and_rot: bool = False, compute_effect_diagnostics: bool = False, robust_loss: str = "mse", huber_delta: float = 1.0, contact_mask_source: str = "gt", contact_mix_max_alpha: float = 0.5, use_full_id_gt_for_torque: bool = False):
    @jax.jit
...
def plot_predictions(train_batch, train_pred, val_batch, val_pred, normalizers, epoch, output_dir, 
    ...
    def _loss_summary(metrics_dict, weights_dict):
        ...
        comps = [
            ...
            ("torque_loss", "torque"),
            ("torque_knee_adduction_loss", "torque_knee_adduction"),
...
def plot_loss_history(
    ...
    COMPONENT_CFG = [
        ...
        ("torque_knee_adduction_loss", "torque_knee_adduction", "Tau KAM", "#8c564b"),
...
def main():
    ...
    parser.add_argument("--warmup_frac", type=float, default=0.03,
                        help="Fraction of total optimizer steps spent warming the LR up to peak "
                             "(--learning_rate) before cosine decay. Default: 0.03.")
    parser.add_argument("--end_lr_frac", type=float, default=0.05,
                        help="Final LR as a fraction of peak at the end of cosine decay. Default: 0.05.")
    parser.add_argument("--no_lr_schedule", action="store_true",
                        help="Disable the warmup+cosine-decay LR schedule and use a constant LR "
    ...
    parser.add_argument("--no_plots", action="store_true",
                        help="Disable ALL plotting (prediction panels, loss history, validation "
                             "outlier grids, best-model panel). Fastest for ablations/sweeps that only "
                             "need scalar metrics.")
    parser.add_argument("--max_subjects", type=int, default=0,
                        help="If >0, restrict training/validation to the first N subjects (after a "
                             "fixed-seed shuffle) for a fast subset run. 0 = use all subjects (default).")
    parser.add_argument("--window_split_frac", type=float, default=0.0,
                        help="If >0, enable WINDOW-LEVEL random splitting: pool windows from ALL "
                             "subjects, keep this fraction of them, and split at the window level "
                             "(mixing subjects). 0 = disabled -> subject-level split (default).")
    parser.add_argument("--window_train_frac", type=float, default=0.7,
                        help="Fraction of the sampled windows used for training in window-split mode "
                             "(the rest are validation). Default: 0.7.")
    parser.add_argument("--window_split_seed", type=int, default=42,
                        help="Seed for the deterministic per-window train/val assignment. Default: 42.")
    parser.add_argument("--quiet_steps", action="store_true",
                        help="Suppress per-step loss prints during training/validation; only print the "
                             "end-of-epoch summary (faster, cleaner logs).")
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=(
            "Correct legacy KneeToCOP_Vectors.npy in memory by subtracting ankle world-Z "
            "from the right/left vector Z components. Do not use after reprocessing with the "
    ...
    parser.add_argument(
        ...
        type=str,
        default="first_step_ratio",
        choices=["first_step_ratio", "absolute"],
        help="How to interpret Torque_weight_knee_adduction.",
    )
    ...
    parser.add_argument(
        ...
        type=_parse_optional_bool_arg,
        help=(
            "Map predicted COP/GRF to joint torques and predicted-COP KAM using MoCap/GT "
            "Jacobian, WorldToGroundAlignedCalcnRotation, and ankle/knee global positions "
            "instead of the video/input-source kinematics. Model inputs stay video/noised; "
            "only the physics reconstruction path switches (default: False)."
        ),
    )
    ...
    parser.add_argument("--effect_diagnostics", type=lambda x: (str(x).lower() == 'true'), default=False,
                        help="Compute the logging-only Tau->COP/Tau->GRF gradient-ratio diagnostics. "
                             "Also reports weighted TauKAM/TauGRF gradient ratios on COP and GRF. "
                             "Adds extra jax.grad passes per train/eval step and does NOT affect the "
                             "optimized loss; leave False for faster training (default: False).")
    parser.add_argument("--robust_loss", type=str, default="mse", choices=["mse", "huber"],
                        help="Primary regression loss for COP/GRF/moments. 'huber' is robust to "
                             "heel-strike transients/artifacts (default: mse, preserves prior behavior).")
    parser.add_argument("--huber_delta", type=float, default=1.0,
                        help="Huber transition point in Z-scored units (only used with --robust_loss huber). Default: 1.0.")
    parser.add_argument("--normalizer_max_batches", type=int, default=100,
                        help="Number of training batches sampled to estimate Z-score normalizers. "
                             "Larger is more representative but slower to start (default: 100).")
    parser.add_argument("--normalizer_stance_only", type=lambda x: (str(x).lower() == 'true'), default=False,
                        help="Estimate COP/GRF Z-score stats over stance (in-contact) frames only, so "
                             "swing-phase zeros do not bias the mean/std (default: False).")
    parser.add_argument("--contact_mask_source", type=str, default="gt", choices=["gt", "pred", "mixed"],
                        help="Contact mask used for the torque reconstruction path. 'gt' (default) "
                             "reproduces prior behavior; 'pred' gates by the model's own predicted contact; "
                             "'mixed' anneals from GT to predicted over training (closes the train/inference "
                             "contact-masking skew). Direct COP/GRF supervision always stays GT-masked.")
    parser.add_argument("--contact_mix_max_alpha", type=float, default=0.5,
                        help="Max blend weight toward predicted contact at the end of training when "
                             "--contact_mask_source mixed (0=all GT, 1=all predicted). Default: 0.5.")
    parser.add_argument("--subject_film", type=lambda x: (str(x).lower() == 'true'), default=False,
                        help="Enable per-layer FiLM conditioning on the static/anthropometry token "
                             "(exploratory architecture change; default False = unchanged architecture). "
                             "Checkpoints trained with this ON must also be loaded with it ON for inference.")
    parser.add_argument("--best_model_metric", type=str, default="val_loss",
                        ...
                        help="Metric used to pick best_model.pkl. 'val_loss' (default) = weighted "
                             "composite val loss (prior behavior); 'torque_rmse' = weighted torque "
                             "score (same as --BestModelByTorque); 'moment_mae_bwh' = physical free-moment "
                             "MAE in %%BW·h; 'joint_moment_mae' = mean bilateral stance MAE (%%BW·h) across "
                             "ankle, hip flexion, hip adduction, knee flexion, and knee adduction (KAM). "
                             "--BestModelByTorque overrides this.")
    ...
    # Optional fast-subset cap: keep only the first N subjects (after the fixed-seed
    # shuffle, so the subset is deterministic and identical across runs). Used by
    # ablation sweeps for a quick directional signal. 0/None => use all subjects.
    if getattr(args, "max_subjects", None):
        _cap = int(args.max_subjects)
        if _cap > 0 and _cap < len(subjects):
            subjects = subjects[:_cap]
            print(f"   ⚡ max_subjects={_cap}: restricted to {len(subjects)} subjects for this run", flush=True)
    ...
    window_split_mode = float(getattr(args, "window_split_frac", 0.0) or 0.0) > 0.0
    if window_split_mode:
        # Window-level random split: both loaders receive the SAME pooled trials from
        # all (selected) subjects; the disjoint train/val window subsets are chosen
        # deterministically inside TrialDataLoader by window_split_role.
        pool_trials = []
        for s in subjects:
            pool_trials.extend(subject_to_trials[s])
        train_trials = list(pool_trials)
        val_trials = list(pool_trials)
        train_subs = list(subjects)
        val_subs = list(subjects)
        print(
            f"   🎲 Window-split mode: pooling {len(pool_trials)} trials from {len(subjects)} subjects; "
            f"keeping {args.window_split_frac:.0%} of windows, "
            f"{args.window_train_frac:.0%}/{1.0 - args.window_train_frac:.0%} train/val at the window level "
            f"(seed {args.window_split_seed}).",
            flush=True,
        )
    elif len(subjects) >= 2:
    ...
    split_info = {
        ...
        "split_mode": (
            "window_random_split" if window_split_mode
            else ("fixed_val_subjects" if fixed_val_requested_subjects else "subject_random_80_20")
        ),
        "window_split": ({
            "window_sample_frac": float(args.window_split_frac),
            "window_train_frac": float(args.window_train_frac),
            "window_split_seed": int(args.window_split_seed),
        } if window_split_mode else None),
    ...
    train_loader = TrialDataLoader(
        ...
        subtract_ankle_height_knee_vecs=args.subtractAnkleHeightKneeVecs,
        ...
        window_split_role=("train" if window_split_mode else None),
        window_sample_frac=args.window_split_frac,
        window_train_frac=args.window_train_frac,
        window_split_seed=args.window_split_seed,
    )
    ...
    val_loader = TrialDataLoader(
        ...
        prediction_margin_frames=args.prediction_margin_frames,
        subtract_ankle_height_knee_vecs=args.subtractAnkleHeightKneeVecs,
        window_split_role=("val" if window_split_mode else None),
        window_sample_frac=args.window_split_frac,
        window_train_frac=args.window_train_frac,
        window_split_seed=args.window_split_seed,
    )
    ...
    normalizers = compute_normalizers_from_loader(
        train_loader,
        max_batches=args.normalizer_max_batches,
        stance_only=args.normalizer_stance_only,
    )
    ...
    model = KinematicsToCOPGRFMoments(
        ...
        use_film=args.subject_film,
    )
    print(f"✅ Model: KinematicsToCOPGRFMoments (Transformer){' + FiLM' if args.subject_film else ''}", flush=True)
    ...
    print(
        ...
        f"use_gt_jacob_and_rot={bool(args.use_gt_jacob_and_rot)}",
        flush=True,
    )
    ...
    # Total optimizer steps drive the warmup -> cosine-decay LR schedule.
    steps_per_epoch = len(train_loader)
    total_optimizer_steps = max(1, steps_per_epoch * int(args.epochs))
    use_lr_schedule = not args.no_lr_schedule
    state, lr_fn = create_train_state(
        init_rng, model, (1, args.window_size, input_dim), (1, static_dim),
    ...
    if use_lr_schedule:
        _warmup_steps = max(1, int(args.warmup_frac * total_optimizer_steps))
        print(f"   LR schedule: warmup {_warmup_steps} steps -> peak {args.learning_rate:.2e} "
              f"-> cosine decay to {args.learning_rate * args.end_lr_frac:.2e} over "
              f"{total_optimizer_steps} steps ({steps_per_epoch} steps/epoch x {args.epochs} epochs)", flush=True)
    else:
        print(f"   LR schedule DISABLED: constant LR {args.learning_rate:.2e}", flush=True)
    ...
    train_step = make_train_step(
        ...
        use_grf_norm_cop=args.UseGRFNormCOP,
        use_gt_jacob_and_rot=bool(args.use_gt_jacob_and_rot),
        compute_effect_diagnostics=args.effect_diagnostics,
        robust_loss=args.robust_loss,
        huber_delta=args.huber_delta,
        contact_mask_source=args.contact_mask_source,
        contact_mix_max_alpha=args.contact_mix_max_alpha,
    )
    ...
    loss_weights = {
        ...
        "torque_knee_adduction": args.Torque_weight_knee_adduction,
        "grf_correction": args.grf_correction_weight,
        "output_reg": 0.0,
    }
    knee_adduction_weight_mode = str(args.Torque_weight_knee_adduction_mode)
    requested_knee_adduction_weight = float(args.Torque_weight_knee_adduction)
    knee_adduction_weight_calibrated = (
        knee_adduction_weight_mode == "absolute" or requested_knee_adduction_weight == 0.0
    )
    effective_knee_adduction_weight = float(loss_weights["torque_knee_adduction"])
    if knee_adduction_weight_mode == "first_step_ratio":
        loss_weights["torque_knee_adduction"] = 0.0
        effective_knee_adduction_weight = 0.0
        if requested_knee_adduction_weight > 0.0:
            print(
                "   Knee adduction loss mode: first_step_ratio "
                f"(target ratio={requested_knee_adduction_weight:.6g}); "
                "absolute multiplier will be calibrated on the first training batch.",
                flush=True,
            )
        else:
            print("   Knee adduction loss disabled (first_step_ratio target is 0).", flush=True)
    else:
        print(
            f"   Knee adduction loss mode: absolute multiplier={effective_knee_adduction_weight:.6g}",
            flush=True,
        )
    ...
    best_moment_mae_bwh = float("inf")  # lower is better (%BW·h) — Plan 6 selection
    best_joint_moment_mae_bwh = float("inf")  # lower is better — ankle/hip/knee+KAM avg
    ...
    train_component_history = {
        ...
        "torque_loss": [], "torque_knee_adduction_loss": [], "torque_cop_effect_loss": [], "torque_grf_effect_loss": [], "grf_correction_loss": [], "output_reg_loss": [],
    }
    ...
    val_rmse_history = {"cop": [], "grf": [], "moments": [], "moment_mae_bwh": [],
                        "knee_adduction_mae_bwh": [], "joint_moment_mae_bwh": []}
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        _cur_lr = float(lr_fn(state.step))
        print(f"   Current LR: {_cur_lr:.3e} (global step {int(state.step)})", flush=True)
        print(f"{'='*70}", flush=True)
        ...
        train_metrics = {
            ...
            "torque_loss": 0, "torque_knee_adduction_loss": 0, "torque_cop_effect_loss": 0, "torque_grf_effect_loss": 0,
            "torque_cop_grad_rms": 0, "torque_grf_grad_rms": 0,
            "torque_knee_adduction_cop_grad_rms": 0, "torque_knee_adduction_grf_grad_rms": 0,
            "torque_knee_adduction_to_torque_cop_grad_ratio": 0, "torque_knee_adduction_to_torque_grf_grad_ratio": 0,
            "grf_correction_loss": 0, "output_reg_loss": 0, "total_loss": 0
        }
        _KEY_TAU_DOFS = {
        ...
        for batch in train_loader:
            ...
            if (
                (not knee_adduction_weight_calibrated)
                and knee_adduction_weight_mode == "first_step_ratio"
                and requested_knee_adduction_weight > 0.0
            ):
                ...
                calibration_scaled_torque = float(loss_weights["torque"]) * calibration_torque_loss
                if np.isfinite(calibration_kam_loss) and calibration_kam_loss > 0.0:
                    effective_knee_adduction_weight = (
                        requested_knee_adduction_weight
                        * calibration_scaled_torque
                        / calibration_kam_loss
                    )
                    loss_weights["torque_knee_adduction"] = float(effective_knee_adduction_weight)
                    print(
                        "   Calibrated knee adduction loss: "
                        f"target_ratio={requested_knee_adduction_weight:.6g}, "
                        f"raw_tau={calibration_torque_loss:.6g}, "
                        f"raw_kam={calibration_kam_loss:.6g}, "
                        f"scaled_tau={calibration_scaled_torque:.6g}, "
                        f"absolute_multiplier={effective_knee_adduction_weight:.6g}, "
                        f"initial_scaled_kam={effective_knee_adduction_weight * calibration_kam_loss:.6g}",
                        flush=True,
                    )
                else:
                    effective_knee_adduction_weight = 0.0
                    loss_weights["torque_knee_adduction"] = 0.0
                    print(
                        "   ⚠️  Could not calibrate knee adduction loss because first-batch "
                        f"raw_kam={calibration_kam_loss}; disabling KAM loss.",
                        flush=True,
                    )
                knee_adduction_weight_calibrated = True
            ...
            # Progress update
            if (not args.quiet_steps) and (train_step_count % log_every == 0 or train_step_count == 1):
                ...
                avg_trq_kam = train_metrics["torque_knee_adduction_loss"] / train_step_count
                ...
                avg_kam_to_tau_cop = train_metrics["torque_knee_adduction_to_torque_cop_grad_ratio"] / train_step_count
                avg_kam_to_tau_grf = train_metrics["torque_knee_adduction_to_torque_grf_grad_ratio"] / train_step_count
                ...
                print(f"      [Avg Raw]    COP:{avg_cop:.4f}, GRF:{avg_grf:.4f}, Mom:{avg_mom:.4f}, Cont:{train_metrics['contact_loss']/train_step_count:.4f}, TauGRF:{avg_trq:.4f}, TauKAM:{avg_trq_kam:.4f}, Cor:{avg_grf_corr:.4f}, OutReg:{avg_out_reg:.4f}")
                print(f"      [Avg Raw]    Tau->COP:{avg_trq_cop:.4f}, Tau->GRF:{avg_trq_grf:.4f}")
                print(f"      [Avg Grad]   TauKAM/TauGRF COP:{avg_kam_to_tau_cop:.4f}, GRF:{avg_kam_to_tau_grf:.4f}")
                print(f"      [Avg Scaled] COP:{avg_cop*loss_weights['cop']:.4f}, GRF:{avg_grf*loss_weights['grf']:.4f}, Mom:{avg_mom*loss_weights['moments']:.4f}, Cont:{train_metrics['contact_loss']/train_step_count*loss_weights['contact']:.4f}, TauGRF:{avg_trq*loss_weights['torque']:.4f}, TauKAM:{avg_trq_kam*loss_weights['torque_knee_adduction']:.4f}, Cor:{avg_grf_corr*loss_weights['grf_correction']:.4f}, OutReg:{avg_out_reg*loss_weights['output_reg']:.4f}")
        ...
        _val_mom_sumae_bwh = np.zeros(2, dtype=np.float64)  # free-moment MAE in %BW·h (Plan 6 selection)
        ...
        # Knee Adduction Moment (KAM) stance MAE accumulators (%BW·h), per side.
        _val_kam_mae_pct_sum = {"R": 0.0, "L": 0.0}
        _val_kam_mae_pct_count = {"R": 0, "L": 0}
        ...
        for batch in val_loader:
            ...
            if "trial_idx" in raw_batch:
                ...
                # Match the loss/eval physics path: when use_gt_jacob_and_rot is on,
                # reconstruct torque/KAM with MoCap rotation + ankle/knee + Jacobians.
                _use_gt_kin = bool(args.use_gt_jacob_and_rot)
                if _use_gt_kin and raw_batch.get("gt_rot_w_to_ga") is not None:
                    rotation_phys_for_recon = project_rotation_matrices(
                        np.asarray(raw_batch["gt_rot_w_to_ga"], dtype=np.float32),
                        xp=np,
                    ).astype(np.float32, copy=False)
                else:
                    rotation_phys_for_recon = rotation_pred_phys
                if _use_gt_kin and raw_batch.get("gt_ankle_pos") is not None and raw_batch.get("gt_knee_pos") is not None:
                    _kam_ankle_pos = raw_batch["gt_ankle_pos"]
                    _kam_knee_pos = raw_batch["gt_knee_pos"]
                else:
                    _kam_ankle_pos = raw_batch.get("ankle_pos")
                    _kam_knee_pos = raw_batch.get("knee_pos")
                ...
                # Free-moment MAE in %BW·h: the moment ratios are already Nm/(m·g·h),
                # so |pred_ratio - gt_ratio| * 100 is a percent-of-BW·h error.
                _mom_err_ratio = moments_pred_ratio - np.array(raw_batch["moments"])
                _val_mom_sumae_bwh += np.sum(np.abs(_mom_err_ratio) * _val_valid, axis=(0, 1)) * 100.0
                ...
                # --- Knee Adduction Moment (KAM) stance MAE in %BW·h ---
                # Mirrors infer.py: KAM = z_vec*GRF_y - y_vec*GRF_z, where (y_vec, z_vec)
                # are the frontal-plane knee->COP vector components. knee_to_cop columns are
                # [Rx,Ry,Rz,Lx,Ly,Lz]; GRF columns [Rx,Ry,Rz,Lx,Ly,Lz]. Normalized to %BW·h
                # with m*g*h and accumulated over each foot's stance frames.
                try:
                    _knee_cop = raw_batch.get("knee_to_cop_vectors")
                    if _knee_cop is not None and _kam_ankle_pos is not None and _kam_knee_pos is not None:
                        _knee_cop = np.asarray(_knee_cop)  # (B, T, 6)
                        _pred_knee_cop = np.asarray(compute_predicted_knee_to_cop_vectors(
                            jnp.asarray(cop_pred_phys),
                            jnp.asarray(_kam_ankle_pos),
                            jnp.asarray(_kam_knee_pos),
                            jnp.asarray(rotation_phys_for_recon),
                        ))
                        _kam_norm = np.maximum(m_batch * 9.8067 * h_batch, 1e-8)[..., 0]  # (B, 1)
                        _kam_r_pred = _pred_knee_cop[..., 2] * grf_pred_phys[..., 1] - _pred_knee_cop[..., 1] * grf_pred_phys[..., 2]
                        _kam_r_gt = _knee_cop[..., 2] * grf_gt_phys[..., 1] - _knee_cop[..., 1] * grf_gt_phys[..., 2]
                        _kam_l_pred = _pred_knee_cop[..., 5] * grf_pred_phys[..., 4] - _pred_knee_cop[..., 4] * grf_pred_phys[..., 5]
                        _kam_l_gt = _knee_cop[..., 5] * grf_gt_phys[..., 4] - _knee_cop[..., 4] * grf_gt_phys[..., 5]
                        _kam_r_err = np.abs((_kam_r_pred - _kam_r_gt) / _kam_norm) * 100.0  # (B, T)
                        _kam_l_err = np.abs((_kam_l_pred - _kam_l_gt) / _kam_norm) * 100.0
                        _val_kam_mae_pct_sum["R"] += float(np.sum(_kam_r_err * _stance_r))
                        _val_kam_mae_pct_count["R"] += int(np.sum(_stance_r))
                        _val_kam_mae_pct_sum["L"] += float(np.sum(_kam_l_err * _stance_l))
                        _val_kam_mae_pct_count["L"] += int(np.sum(_stance_l))
                except Exception:
                    pass  # never crash training over a diagnostic
                ...
                try:
                    _full_mom_pred = np.array(compute_full_external_moments(
                        ...
                        jnp.array(moments_pred_phys),
                        raw_batch["ankle_heights"],
                        jnp.array(rotation_phys_for_recon),
                    ))
                    if _use_gt_kin and raw_batch.get("gt_jacp") is not None and raw_batch.get("gt_jacr") is not None:
                        _jacp_pred = np.asarray(raw_batch["gt_jacp"])
                        _jacr_pred = np.asarray(raw_batch["gt_jacr"])
                    else:
                        _jacp_pred, _jacr_pred = select_torque_jacobians(
                            pred_np,
                            raw_batch,
            ...
            # Progress update
            if (not args.quiet_steps) and (val_step_count % val_log_every == 0 or val_step_count == 1):
                ...
                avg_trq_kam = val_metrics["torque_knee_adduction_loss"] / val_step_count
                ...
                avg_kam_to_tau_cop = val_metrics["torque_knee_adduction_to_torque_cop_grad_ratio"] / val_step_count
                avg_kam_to_tau_grf = val_metrics["torque_knee_adduction_to_torque_grf_grad_ratio"] / val_step_count
                ...
                print(f"      [Avg Raw]    COP:{avg_cop:.4f}, GRF:{avg_grf:.4f}, Mom:{avg_mom:.4f}, Cont:{val_metrics['contact_loss']/val_step_count:.4f}, TauGRF:{avg_trq:.4f}, TauKAM:{avg_trq_kam:.4f}, Cor:{avg_grf_corr:.4f}, OutReg:{avg_out_reg:.4f}")
                print(f"      [Avg Raw]    Tau->COP:{avg_trq_cop:.4f}, Tau->GRF:{avg_trq_grf:.4f}")
                print(f"      [Avg Grad]   TauKAM/TauGRF COP:{avg_kam_to_tau_cop:.4f}, GRF:{avg_kam_to_tau_grf:.4f}")
                print(f"      [Avg Scaled] COP:{avg_cop*loss_weights['cop']:.4f}, GRF:{avg_grf*loss_weights['grf']:.4f}, Mom:{avg_mom*loss_weights['moments']:.4f}, Cont:{val_metrics['contact_loss']/val_step_count*loss_weights['contact']:.4f}, TauGRF:{avg_trq*loss_weights['torque']:.4f}, TauKAM:{avg_trq_kam*loss_weights['torque_knee_adduction']:.4f}, Cor:{avg_grf_corr*loss_weights['grf_correction']:.4f}, OutReg:{avg_out_reg*loss_weights['output_reg']:.4f}")
                print(f"      [Avg Scaled] Tau->COP:{avg_trq_cop*loss_weights['torque']:.4f}, Tau->GRF:{avg_trq_grf*loss_weights['torque']:.4f}")
                print("", flush=True)
        ...
        if (not args.no_plots) and (not args.disable_validation_outlier_plots) and top_outliers_by_channel and (epoch == 1 or epoch % 5 == 0):
        ...
        print(f"      [Raw]    COP:{train_metrics['cop_loss']:.4f}, GRF:{train_metrics['grf_loss']:.4f}, Mom:{train_metrics['moments_loss']:.4f}, Cont:{train_metrics['contact_loss']:.4f}, TauGRF:{train_metrics['torque_loss']:.4f}, TauKAM:{train_metrics['torque_knee_adduction_loss']:.4f}, Cor:{train_metrics['grf_correction_loss']:.4f}, OutReg:{train_metrics['output_reg_loss']:.4f}")
        print(f"      [Raw]    Tau->COP:{train_metrics['torque_cop_effect_loss']:.4f}, Tau->GRF:{train_metrics['torque_grf_effect_loss']:.4f}")
        print(f"      [Grad]   TauKAM/TauGRF COP:{train_metrics['torque_knee_adduction_to_torque_cop_grad_ratio']:.4f}, GRF:{train_metrics['torque_knee_adduction_to_torque_grf_grad_ratio']:.4f}")
        print(f"      [Scaled] COP:{train_metrics['cop_loss']*loss_weights['cop']:.4f}, GRF:{train_metrics['grf_loss']*loss_weights['grf']:.4f}, Mom:{train_metrics['moments_loss']*loss_weights['moments']:.4f}, Cont:{train_metrics['contact_loss']*loss_weights['contact']:.4f}, TauGRF:{train_metrics['torque_loss']*loss_weights['torque']:.4f}, TauKAM:{train_metrics['torque_knee_adduction_loss']*loss_weights['torque_knee_adduction']:.4f}, Cor:{train_metrics['grf_correction_loss']*loss_weights['grf_correction']:.4f}, OutReg:{train_metrics['output_reg_loss']*loss_weights['output_reg']:.4f}")
        ...
        print(f"      [Raw]    COP:{float(val_metrics['cop_loss']):.4f}, GRF:{float(val_metrics['grf_loss']):.4f}, Mom:{float(val_metrics['moments_loss']):.4f}, Cont:{float(val_metrics['contact_loss']):.4f}, TauGRF:{float(val_metrics['torque_loss']):.4f}, TauKAM:{float(val_metrics['torque_knee_adduction_loss']):.4f}, Cor:{float(val_metrics['grf_correction_loss']):.4f}, OutReg:{float(val_metrics['output_reg_loss']):.4f}")
        print(f"      [Raw]    Tau->COP:{float(val_metrics['torque_cop_effect_loss']):.4f}, Tau->GRF:{float(val_metrics['torque_grf_effect_loss']):.4f}")
        print(f"      [Grad]   TauKAM/TauGRF COP:{float(val_metrics['torque_knee_adduction_to_torque_cop_grad_ratio']):.4f}, GRF:{float(val_metrics['torque_knee_adduction_to_torque_grf_grad_ratio']):.4f}")
        print(f"      [Scaled] COP:{val_metrics['cop_loss']*loss_weights['cop']:.4f}, GRF:{val_metrics['grf_loss']*loss_weights['grf']:.4f}, Mom:{val_metrics['moments_loss']*loss_weights['moments']:.4f}, Cont:{val_metrics['contact_loss']*loss_weights['contact']:.4f}, TauGRF:{val_metrics['torque_loss']*loss_weights['torque']:.4f}, TauKAM:{val_metrics['torque_knee_adduction_loss']*loss_weights['torque_knee_adduction']:.4f}, Cor:{val_metrics['grf_correction_loss']*loss_weights['grf_correction']:.4f}, OutReg:{val_metrics['output_reg_loss']*loss_weights['output_reg']:.4f}")
        ...
        _fs_moment_mae_bwh = float("nan")
        _fs_knee_adduction_mae_bwh = float("nan")   # bilateral KAM stance MAE, %BW·h
        _fs_joint_moment_mae_bwh = float("nan")     # mean of ankle/hip_flex/hip_add/knee/KAM, %BW·h
        if _val_frames > 0:
            n = float(_val_frames)
            ...
            _fs_moment_mae_bwh = float(np.mean(_val_mom_sumae_bwh) / n)  # bilateral free-moment MAE, %BW·h
            ...
            # Knee Adduction Moment (KAM) bilateral stance MAE (%BW·h).
            _kam_sum_lr = _val_kam_mae_pct_sum["R"] + _val_kam_mae_pct_sum["L"]
            _kam_cnt_lr = _val_kam_mae_pct_count["R"] + _val_kam_mae_pct_count["L"]
            _fs_knee_adduction_mae_bwh = float(_kam_sum_lr / _kam_cnt_lr) if _kam_cnt_lr > 0 else float("nan")
            _val_stance_tau_mae_pct_norm_bilateral["knee_adduction"] = _fs_knee_adduction_mae_bwh
            # Composite best-model metric: mean of ankle, hip flexion, hip adduction, knee
            # flexion, and knee adduction (KAM) — each a bilateral stance MAE in %BW·h.
            _sel_joint_vals = [
                _val_stance_tau_mae_pct_norm_bilateral.get(_j, float("nan"))
                for _j in ("ankle", "hip_flexion", "hip_adduction", "knee", "knee_adduction")
            ]
            _sel_finite = [v for v in _sel_joint_vals if v == v]  # drop NaNs
            _fs_joint_moment_mae_bwh = float(np.mean(_sel_finite)) if _sel_finite else float("nan")
        ...
        if np.isfinite(_fs_moment_mae_bwh):
            print(f"   Val free-moment MAE: {_fs_moment_mae_bwh:.4f} %BW·h", flush=True)
        if np.isfinite(_fs_knee_adduction_mae_bwh):
            print(f"   Val knee-adduction (KAM) MAE: {_fs_knee_adduction_mae_bwh:.4f} %BW·h", flush=True)
        if np.isfinite(_fs_joint_moment_mae_bwh):
            print(f"   Val joint-moment MAE (ankle/hip_flex/hip_add/knee/KAM avg): "
                  f"{_fs_joint_moment_mae_bwh:.4f} %BW·h", flush=True)
        ...
        val_rmse_history["moment_mae_bwh"].append(float(_fs_moment_mae_bwh))
        val_rmse_history["knee_adduction_mae_bwh"].append(float(_fs_knee_adduction_mae_bwh))
        val_rmse_history["joint_moment_mae_bwh"].append(float(_fs_joint_moment_mae_bwh))
        ...
        _wandb_log = {
            ...
            "train/learning_rate": float(lr_fn(state.step)),
            ...
            "train/torque_knee_adduction_loss": float(train_metrics["torque_knee_adduction_loss"]),
            ...
            "train/torque_cop_grad_rms": float(train_metrics["torque_cop_grad_rms"]),
            "train/torque_grf_grad_rms": float(train_metrics["torque_grf_grad_rms"]),
            "train/torque_knee_adduction_cop_grad_rms": float(train_metrics["torque_knee_adduction_cop_grad_rms"]),
            "train/torque_knee_adduction_grf_grad_rms": float(train_metrics["torque_knee_adduction_grf_grad_rms"]),
            "train/torque_knee_adduction_to_torque_cop_grad_ratio": float(train_metrics["torque_knee_adduction_to_torque_cop_grad_ratio"]),
            "train/torque_knee_adduction_to_torque_grf_grad_ratio": float(train_metrics["torque_knee_adduction_to_torque_grf_grad_ratio"]),
            ...
            "val/torque_knee_adduction_loss": float(val_metrics["torque_knee_adduction_loss"]),
            ...
            "val/torque_cop_grad_rms": float(val_metrics["torque_cop_grad_rms"]),
            "val/torque_grf_grad_rms": float(val_metrics["torque_grf_grad_rms"]),
            "val/torque_knee_adduction_cop_grad_rms": float(val_metrics["torque_knee_adduction_cop_grad_rms"]),
            "val/torque_knee_adduction_grf_grad_rms": float(val_metrics["torque_knee_adduction_grf_grad_rms"]),
            "val/torque_knee_adduction_to_torque_cop_grad_ratio": float(val_metrics["torque_knee_adduction_to_torque_cop_grad_ratio"]),
            "val/torque_knee_adduction_to_torque_grf_grad_ratio": float(val_metrics["torque_knee_adduction_to_torque_grf_grad_ratio"]),
        ...
        _wandb_log["val/moment_mae_bwh"] = float(_fs_moment_mae_bwh)
        _wandb_log["val/knee_adduction_mae_bwh"] = float(_fs_knee_adduction_mae_bwh)
        _wandb_log["val/joint_moment_mae_bwh"] = float(_fs_joint_moment_mae_bwh)
        ...
        if args.no_plots:
            _should_plot_predictions = False
        elif args.save_final_predictions_only:
        ...
        # Selection metric (Plan 6): 'val_loss' (default) | 'torque_rmse' | 'moment_mae_bwh'.
        # The legacy --BestModelByTorque flag maps to 'torque_rmse' for backward compatibility.
        _is_new_best = False
        _sel_metric = "torque_rmse" if args.BestModelByTorque else args.best_model_metric
        if _sel_metric == "torque_rmse" and not np.isnan(_epoch_torque_score):
        ...
        elif _sel_metric == "moment_mae_bwh" and np.isfinite(_fs_moment_mae_bwh):
            # Physical free-moment MAE (%BW·h): stable, weight-independent criterion.
            if _fs_moment_mae_bwh < best_moment_mae_bwh:
                best_moment_mae_bwh = _fs_moment_mae_bwh
                best_val_loss = float(val_metrics["total_loss"])  # keep for reference
                _is_new_best = True
                _best_label = f"moment MAE: {best_moment_mae_bwh:.4f} %BW·h"
        elif _sel_metric == "joint_moment_mae" and np.isfinite(_fs_joint_moment_mae_bwh):
            # Composite joint-moment MAE (%BW·h): mean of ankle, hip flexion, hip
            # adduction, knee flexion, and knee adduction (KAM), each bilateral.
            if _fs_joint_moment_mae_bwh < best_joint_moment_mae_bwh:
                best_joint_moment_mae_bwh = _fs_joint_moment_mae_bwh
                best_val_loss = float(val_metrics["total_loss"])  # keep for reference
                _is_new_best = True
                _best_label = f"joint moment MAE: {best_joint_moment_mae_bwh:.4f} %BW·h"
        else:
            # Default mode (also used as fallback when the selected metric is unavailable).
        ...
        if _is_new_best:
            ...
            with open(os.path.join(args.output_dir, "best_model.pkl"), "wb") as f:
                pickle.dump({
                    ...
                    "best_val_loss": best_val_loss,
                    "best_torque_score": best_torque_score,
                    "best_moment_mae_bwh": best_moment_mae_bwh,
                    "val_moment_mae_bwh": _fs_moment_mae_bwh,
                    "best_joint_moment_mae_bwh": best_joint_moment_mae_bwh,
                    "val_joint_moment_mae_bwh": _fs_joint_moment_mae_bwh,
                    "val_knee_adduction_mae_bwh": _fs_knee_adduction_mae_bwh,
                    "best_model_metric": _sel_metric,
                    "use_film": bool(args.subject_film),
                    "train_cop_rmse_m": _train_cop_overall_rmse,
                    "train_grf_rmse_N": _train_grf_overall_rmse,
                    "train_moments_rmse_Nm": _train_mom_overall_rmse,
                    "val_cop_rmse_m": _fs_cop_overall_rmse,
            ...
            if (
                (not args.no_plots)
                and (not args.save_final_predictions_only)
    ...
    hyperparams = {
        ...
        "subtractAnkleHeightKneeVecs": bool(args.subtractAnkleHeightKneeVecs),
        "input_dim": int(input_dim),
        "static_dim": int(static_dim),
        "window_size": args.window_size,
        "stride": args.stride,
        ...
        "Torque_weight_knee_adduction": args.Torque_weight_knee_adduction,
        "torque_weight_knee_adduction": args.Torque_weight_knee_adduction,
        "Torque_weight_knee_adduction_mode": args.Torque_weight_knee_adduction_mode,
        "effective_Torque_weight_knee_adduction": float(effective_knee_adduction_weight),
    ...
    rmse_history = {
        ...
        "val": {
            ...
            "moment_mae_bwh": val_rmse_history["moment_mae_bwh"],
            "knee_adduction_mae_bwh": val_rmse_history["knee_adduction_mae_bwh"],
            "joint_moment_mae_bwh": val_rmse_history["joint_moment_mae_bwh"],
        },
    }
    ...
    training_summary = _json_compatible(
        {
            ...
            "output_dir": args.output_dir,
            "selection_mode": _sel_metric,
            "selection_metric_name": {
                "torque_rmse": "val/best_torque_score_Nm",
                "moment_mae_bwh": "val/moment_mae_bwh",
                "joint_moment_mae": "val/joint_moment_mae_bwh",
                "val_loss": "val/total_loss",
            }.get(_sel_metric, "val/total_loss"),
            "best_val_loss": best_val_loss,
            "best_torque_score": best_torque_score,
            "best_moment_mae_bwh": best_moment_mae_bwh,
            "best_joint_moment_mae_bwh": best_joint_moment_mae_bwh,
