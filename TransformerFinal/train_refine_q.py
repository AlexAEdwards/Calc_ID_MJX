...
**Default dataset layout** (standalone ``train_refine_q`` / trusted HPO trees):
    ...
    Model input per frame: **(pos, vel, acc)**
        ``pos_inputs_noised`` (standard) or ``pos_inputs`` (Loso cache) -> ``(T, 18)``
...
MIN_TRIAL_LENGTH = 30
POS_INPUT_DIM    = 18
OUTPUT_DIM       = 18   # same column layout as pos_inputs.npy
...
class QRefineDataLoader:
    ...
    def _extract_windows(self, trial_data: Dict) -> List[Dict]:
        pos_noised = trial_data["pos"]     # (T, 18)
        vel_noised = trial_data["vel"]     # (T, 21)
        acc_noised = trial_data["acc"]     # (T, 21)
        pos_gt     = trial_data["pos_gt"]  # (T, 18)
