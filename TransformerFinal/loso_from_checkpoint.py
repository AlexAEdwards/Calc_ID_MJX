...
try:
    ...
    from train_directTorque import (
        make_direct_eval_step,
        make_direct_train_step,
        normalize_direct_batch,
    )
except Exception:
    DIRECT_TORQUE_NAMES = ()
    is_direct_torque_hparams = None
    make_direct_eval_step = None
    make_direct_train_step = None
    normalize_direct_batch = None


def _is_direct_torque_config(config: Mapping[str, Any]) -> bool:
    return bool(is_direct_torque_hparams is not None and is_direct_torque_hparams(config))
...
LOSO_CONFIG: Dict[str, Any] = {
    ...
    "contact_weight": None,
    "torque_weight": 0.0,
    "torque_weight_knee_adduction": 0.0,
    "torque_weight_knee_adduction_mode": "absolute",
    ...
    # Correct legacy KneeToCOP_Vectors.npy in memory. Disable after the OpenCap
    # dataset has been regenerated with the corrected ProcessData.py.
    "subtract_ankle_height_knee_vecs": False,
...
def _load_checkpoint_bundle(checkpoint_path: Path) -> Tuple[Mapping[str, Any], Dict[str, Any]]:
    ...
    if "torque_weight_knee_adduction_mode" not in normalized:
        normalized["torque_weight_knee_adduction_mode"] = raw_hparams.get(
            "Torque_weight_knee_adduction_mode",
            raw_hparams.get("torque_weight_knee_adduction_mode", "absolute"),
        )
...
def _build_loss_weights(config: Mapping[str, Any]) -> Dict[str, float]:
    ...
    if bool(config.get("use_gt_jacob_and_rot_for_training", False)):
        return {
            ...
            "torque_knee_adduction": torque_kam_weight,
            "grf_correction": 0.0,
            "output_reg": 0.0,
        }
    return {
...
def _knee_adduction_weight_mode(config: Mapping[str, Any]) -> str:
    mode = str(
        config.get(
            "torque_weight_knee_adduction_mode",
    ...
    if mode not in {"absolute", "first_step_ratio"}:
        raise ValueError(
            "Torque knee-adduction weight mode must be 'absolute' or "
            f"'first_step_ratio', got {mode!r}."
        )
    return mode


def _configure_kam_loss_weight_for_mode(
    loss_weights: Dict[str, float],
    config: Mapping[str, Any],
    *,
    log_prefix: str,
) -> bool:
    """Prepare KAM loss weight and return True when first-batch calibration is needed."""
    mode = _knee_adduction_weight_mode(config)
    requested = float(loss_weights.get("torque_knee_adduction", 0.0))
    if isinstance(config, MutableMapping):
        config["effective_torque_weight_knee_adduction"] = requested
        config["requested_torque_weight_knee_adduction"] = requested
        config["torque_weight_knee_adduction_mode"] = mode
    if mode == "first_step_ratio":
        loss_weights["torque_knee_adduction"] = 0.0
        if requested > 0.0:
            print(
                f"{log_prefix} Knee adduction loss mode: first_step_ratio "
                f"(target ratio={requested:.6g}); calibrating on first training batch.",
                flush=True,
            )
            return True
        print(f"{log_prefix} Knee adduction loss disabled (first_step_ratio target is 0).", flush=True)
        return False
    print(
        f"{log_prefix} Knee adduction loss mode: absolute multiplier={requested:.6g}",
        flush=True,
    )
    return False


def _calibrate_kam_loss_weight_first_batch(
    *,
    state,
    ...
    requested = float(config.get("requested_torque_weight_knee_adduction", 0.0))
    calibration_batch = next(iter(train_loader), None)
    if calibration_batch is None:
        raise RuntimeError("Cannot calibrate knee adduction loss: training loader yielded zero batches.")
    batch_norm = train_module.normalize_batch(calibration_batch, normalizers)
    calibration_loss_weights = dict(loss_weights)
    calibration_loss_weights["torque_knee_adduction"] = 0.0
    calibration_metrics, _calibration_pred = eval_step(
        state,
        batch_norm,
        calibration_loss_weights,
        float(epoch),
    )
    calibration_torque_loss = float(calibration_metrics["torque_loss"])
    calibration_kam_loss = float(calibration_metrics["torque_knee_adduction_loss"])
    calibration_scaled_torque = float(loss_weights.get("torque", 0.0)) * calibration_torque_loss
    payload = {
        "mode": "first_step_ratio",
        "target_ratio": float(requested),
        "raw_tau": float(calibration_torque_loss),
        "raw_kam": float(calibration_kam_loss),
        "scaled_tau": float(calibration_scaled_torque),
        "absolute_multiplier": 0.0,
        "initial_scaled_kam": 0.0,
    }
    if np.isfinite(calibration_kam_loss) and calibration_kam_loss > 0.0:
        effective = requested * calibration_scaled_torque / calibration_kam_loss
        loss_weights["torque_knee_adduction"] = float(effective)
        config["effective_torque_weight_knee_adduction"] = float(effective)
        payload["absolute_multiplier"] = float(effective)
        payload["initial_scaled_kam"] = float(effective * calibration_kam_loss)
        print(
            f"{log_prefix} Calibrated knee adduction loss: "
            f"target_ratio={requested:.6g}, "
            f"raw_tau={calibration_torque_loss:.6g}, "
            f"raw_kam={calibration_kam_loss:.6g}, "
            f"scaled_tau={calibration_scaled_torque:.6g}, "
            f"absolute_multiplier={effective:.6g}, "
            f"initial_scaled_kam={effective * calibration_kam_loss:.6g}",
            flush=True,
        )
    else:
        loss_weights["torque_knee_adduction"] = 0.0
        config["effective_torque_weight_knee_adduction"] = 0.0
        print(
            f"{log_prefix} ⚠️ Could not calibrate knee adduction loss because "
            f"first-batch raw_kam={calibration_kam_loss}; disabling KAM loss.",
            flush=True,
        )
    config["torque_weight_knee_adduction_calibration"] = payload
    return payload
...
def _apply_cli_overrides(config: Dict[str, Any], args: argparse.Namespace) -> Dict[str, Any]:
    ...
    scalar_override_map = {
        ...
        "Torque_weight_knee_adduction": "torque_weight_knee_adduction",
        "torque_weight_knee_adduction": "torque_weight_knee_adduction",
        "Torque_weight_knee_adduction_mode": "torque_weight_knee_adduction_mode",
        "torque_weight_knee_adduction_mode": "torque_weight_knee_adduction_mode",
    ...
    bool_override_map = {
        ...
        "subtractAnkleHeightKneeVecs": "subtract_ankle_height_knee_vecs",
...
def _write_infer_style_summary_artifacts(
    ...
    if trial_metrics:
        ...
        required_dashboard_keys = {"cop_rmse", "grf_rmse", "moments_rmse", "torque_rmse"}
        if all(required_dashboard_keys.issubset(set(metric.keys())) for metric in trial_metrics):
            infer_module.create_summary_dashboard(list(trial_metrics), str(output_dir))
        else:
            print(
                "   ℹ️ Skipping legacy COP/GRF summary dashboard; "
                "trial metrics do not contain COP/GRF RMSE keys.",
                flush=True,
            )
...
def _evaluate_single_trial_infer_style(
    ...
    *,
    ...
    kam_timeseries_payload = _build_left_kam_timeseries_payload(
        trial_data=trial_data,
        ground_truth=ground_truth,
        fine_tuned_predictions=evaluation_predictions,
        reference_outputs_eval=reference_outputs_eval,
        evaluation_mask=evaluation_mask,
        subtract_ankle_height_knee_vecs=bool(
            config.get("subtract_ankle_height_knee_vecs", False)
        ),
    ...
    return {
        ...
        "left_kam_timeseries": kam_timeseries_payload,
...
def _run_infer_style_evaluation(
    ...
    *,
    ...
    # Standard held-out prediction metrics are reported on NORMAL (non-trunk-sway) trials
    # only. With --includeTrunkSway the held-out subject's TS trials also appear in
    # held_out_trials (so the OTHER subjects' TS trials can be used for fine-tuning), but the
    # trunk-sway TS trials are evaluated separately in _run_trunk_sway_effect_evaluation; keeping
    # them out here prevents the "normal" metrics from being contaminated by the TS condition.
    normal_held_out_trials = [
        trial_info
        for trial_info in fold["held_out_trials"]
        if not str(trial_info.get("subject", "")).endswith("_TS")
    ]

    for trial_info in normal_held_out_trials:
        trial_name = str(trial_info.get("trial_name", _trial_root_from_info(trial_info).name))
        try:
...
def _run_direct_torque_infer_style_evaluation(
    fold: Mapping[str, Any],
    *,
    fold_dir: Path,
    model,
    ...
    for trial_info in normal_held_out_trials:
        trial_name = str(trial_info.get("trial_name", _trial_root_from_info(trial_info).name))
        try:
            trial_result = _evaluate_single_trial_direct_torque_for_ts(
                trial_info,
                model=model,
                final_params=params,
            ...
            if trial_result.get("metrics") is not None:
                trial_metrics.append(dict(trial_result["metrics"]))
            if trial_result.get("mae_report"):
                overall_mae[trial_result["trial_name"]] = {
                    key: float(value) for key, value in dict(trial_result["mae_report"]).items()
                }
            for source_label, source_report in (trial_result.get("reference_mae_reports") or {}).items():
                if source_label == "Original OpenCap PredInput" and source_report:
                    overall_mae_original_opencap_predinput[trial_result["trial_name"]] = {
                        key: float(value) for key, value in dict(source_report).items()
                    }
                elif source_label == "Original Motion Capture" and source_report:
                    overall_mae_original_motion_capture[trial_result["trial_name"]] = {
                        key: float(value) for key, value in dict(source_report).items()
                    }
            for dof_name, stance_result in (trial_result.get("stance_results") or {}).items():
                if not stance_result:
                    continue
                bucket = aggregated_stance_data.setdefault(dof_name, {"pred": [], "gt": []})
                bucket["pred"].append(np.asarray(stance_result["pred"]))
                bucket["gt"].append(np.asarray(stance_result["gt"]))
            ...
            print(f"   ❌ Direct infer-style evaluation failed for {trial_name}: {exc}", flush=True)
            failed_trials.append({"trial_name": trial_name, "error": str(exc)})
        finally:
            gc.collect()
    ...
    if not trial_metrics:
        raise RuntimeError(
            f"Direct infer-style evaluation produced no successful held-out trial metrics for {fold['held_out_subject']}."
        )
    ...
    try:
        _create_direct_torque_summary_dashboard(
            output_dir=infer_output_dir,
            trial_metrics=trial_metrics,
            mae_by_trial=overall_mae,
        )
    except Exception as exc:
        print(f"   ⚠️ Direct torque summary dashboard failed: {exc}", flush=True)
    summary_payload.update(
        {
            "direct_torque": True,
            "output_dir": str(infer_output_dir),
            ...
            "mae_reports": overall_mae,
            "mae_reports_original_opencap_predinput": overall_mae_original_opencap_predinput,
            "mae_reports_original_opencap_ocinput": {},
            "mae_reports_original_motion_capture": overall_mae_original_motion_capture,
            "mae_reports_original_opencap": overall_mae_original_opencap_predinput,
            "mae_reports_original_motioncapture": overall_mae_original_motion_capture,
            "comparison_rows": [],
            "comparison_metric_means": {},
            "comparison_metric_stds": {},
            "failed_trials": failed_trials,
            "trial_artifacts": trial_artifacts,
        }
    )
    _save_json(infer_output_dir / "infer_style_summary.json", summary_payload)
    return summary_payload


def _create_direct_torque_summary_dashboard(
    *,
    output_dir: Path,
    trial_metrics: Sequence[Mapping[str, Any]],
    mae_by_trial: Mapping[str, Mapping[str, float]],
) -> None:
    summary_dir = output_dir / "summary_dashboard"
    summary_dir.mkdir(parents=True, exist_ok=True)
    trial_names = [str(metric.get("trial_name", f"trial_{idx}")) for idx, metric in enumerate(trial_metrics)]
    mae_vals = [float(metric.get("direct_torque_mae_bwh", np.nan)) for metric in trial_metrics]
    rmse_vals = [float(metric.get("direct_torque_rmse_bwh", np.nan)) for metric in trial_metrics]
    kam_vals = [float(metric.get("left_kam_mae_bwh", np.nan)) for metric in trial_metrics]

    fig_table = infer_module.go.Figure(
        data=[
            infer_module.go.Table(
                header=dict(
                    values=["Trial", "Direct Torque MAE (%BW*H)", "Direct Torque RMSE (%BW*H)", "Left KAM MAE (%BW*H)"],
                ...
                cells=dict(
                    values=[
                        trial_names,
                        [f"{value:.4f}" for value in mae_vals],
                        [f"{value:.4f}" for value in rmse_vals],
                        [f"{value:.4f}" for value in kam_vals],
                    ],
                    fill_color="lavender",
                    align="left",
                ),
    ...
    fig_table.update_layout(title="Direct Torque Infer-Style Summary")
    fig_table.write_html(summary_dir / "summary_table.html")

    fig_box = infer_module.make_subplots(
        rows=1,
        cols=3,
        subplot_titles=("Direct Torque MAE", "Direct Torque RMSE", "Left KAM MAE"),
    )
    fig_box.add_trace(infer_module.go.Box(y=mae_vals, name="MAE", boxpoints="all"), row=1, col=1)
    fig_box.add_trace(infer_module.go.Box(y=rmse_vals, name="RMSE", boxpoints="all"), row=1, col=2)
    fig_box.add_trace(infer_module.go.Box(y=kam_vals, name="KAM MAE", boxpoints="all"), row=1, col=3)
    fig_box.update_yaxes(title_text="%BW*H", row=1, col=1)
    fig_box.update_yaxes(title_text="%BW*H", row=1, col=2)
    fig_box.update_yaxes(title_text="%BW*H", row=1, col=3)
    fig_box.update_layout(title_text="Direct Torque Metric Distributions", showlegend=False)
    fig_box.write_html(summary_dir / "metrics_distribution.html")

    if mae_by_trial:
        channel_values: Dict[str, List[float]] = {}
        for mae_report in mae_by_trial.values():
            for name, value in mae_report.items():
                value_f = float(value)
                if np.isfinite(value_f):
                    channel_values.setdefault(str(name), []).append(value_f)
        labels = list(channel_values.keys())
        means = [float(np.mean(channel_values[label])) for label in labels]
        fig_bar = infer_module.go.Figure(
            data=[
                infer_module.go.Bar(
                    x=labels,
                    y=means,
        ...
        fig_bar.update_layout(
            title="Average Direct Torque MAE by Channel",
            xaxis_title="Channel",
            yaxis_title="MAE (%BW*H)",
            xaxis_tickangle=-35,
            template="plotly_white",
        )
        fig_bar.write_html(summary_dir / "direct_torque_channel_mae.html")
...
def _align_1d_or_2d_length(arr: np.ndarray, n_frames: int) -> np.ndarray:
    arr = np.asarray(arr)
    ...
    if arr.shape[0] > n_frames:
        return arr[:n_frames]
    if arr.shape[0] == 0:
        return np.zeros((n_frames,) + arr.shape[1:], dtype=arr.dtype)
    pad_shape = [(0, n_frames - arr.shape[0])] + [(0, 0)] * (arr.ndim - 1)
    return np.pad(arr, pad_shape, mode="edge")


def _load_knee_cop_vectors_for_kam(
    ...
    *,
    ...
    """Load the KAM moment-arm (knee->COP) vectors for a given kinematics ``source``.

    Ground-truth KAM uses ``source="mocap"`` (the true kinematics). A prediction's
    reconstructed KAM uses the moment arm of ITS OWN input source ("mocap" for MoCap
    input, "video"/"processed" otherwise). Falls back to MoCap, then the trial's input
    processed dir, when the requested source's dir is unavailable.
    """
    src = str(source).lower()
    trial_dir = Path(str(trial_data.get("trial_dir", "")))
    bases = [
        infer_module.source_processed_dir(trial_dir, src),
        infer_module.source_processed_dir(trial_dir, "mocap"),
    ]
    ipd = trial_data.get("input_processed_dir")
    if ipd:
        bases.append(Path(str(ipd)))
    for base in bases:
        for name in ("KneeToCOP_Vectors_Mocap.npy", "KneeToCOP_Vectors.npy"):
            path = base / name
            if path.exists():
                vectors = np.asarray(np.load(path), dtype=np.float32)
                if vectors.ndim == 2 and vectors.shape[1] >= 6:
                    vectors = _align_1d_or_2d_length(vectors, n_frames)
                    if subtract_ankle_height_knee_vecs:
                        ankle_key = "gt_ankle_pos" if src == "mocap" else "ankle_pos"
                        ankle_pos = trial_data.get(ankle_key)
                        if ankle_pos is None:
                            raise ValueError(
                                "--subtractAnkleHeightKneeVecs requires matching ankle positions "
                                f"for KAM evaluation: {path}"
                            )
                        ankle = _align_1d_or_2d_length(
                            np.asarray(ankle_pos, dtype=np.float32), n_frames
                        )
...
def _left_kam_from_grf_percent_bwh(
    *,
    knee_cop_vectors: np.ndarray,
    grf: np.ndarray,
    norm_factor: float,
) -> np.ndarray:
    grf = _align_1d_or_2d_length(np.asarray(grf, dtype=np.float32), len(knee_cop_vectors))
    z_vec_l = knee_cop_vectors[:, 5]
    y_vec_l = knee_cop_vectors[:, 4]
    grf_y_l = grf[:, 4]
    grf_z_l = grf[:, 5]
    return ((z_vec_l * grf_y_l) - (y_vec_l * grf_z_l)) / float(norm_factor) * 100.0


def _build_left_kam_timeseries_payload(
    *,
    trial_data: Mapping[str, Any],
    ground_truth: Mapping[str, Any],
    fine_tuned_predictions: Mapping[str, Any],
    reference_outputs_eval: Mapping[str, Any],
    evaluation_mask: np.ndarray,
    subtract_ankle_height_knee_vecs: bool = False,
) -> Optional[Dict[str, Any]]:
    """Compact full-trial KAM payload used for post-fold stance segmentation."""
    grf_gt = np.asarray(ground_truth.get("grf"), dtype=np.float32)
    if grf_gt.ndim != 2 or grf_gt.shape[1] < 6:
        return None
    n_frames = int(grf_gt.shape[0])
    # Ground-truth KAM uses the measured MoCap moment arm. Predicted KAM should use
    # the model-predicted COP arm when inference provides it, falling back to the
    # older input-source measured arm only for legacy payloads.
    knee_cop_gt = _load_knee_cop_vectors_for_kam(
        trial_data,
        n_frames,
        source="mocap",
        subtract_ankle_height_knee_vecs=subtract_ankle_height_knee_vecs,
    )
    if knee_cop_gt is None:
        return None
    input_source = str(trial_data.get("input_source", "processed")).lower()
    knee_cop_pred = _load_knee_cop_vectors_for_kam(
        trial_data,
        n_frames,
        source=input_source,
        subtract_ankle_height_knee_vecs=subtract_ankle_height_knee_vecs,
    ...
    if knee_cop_pred is None:
        knee_cop_pred = knee_cop_gt
    knee_cop_video = _load_knee_cop_vectors_for_kam(
        trial_data,
        n_frames,
        source="video",
        subtract_ankle_height_knee_vecs=subtract_ankle_height_knee_vecs,
    )
    if knee_cop_video is None:
        knee_cop_video = knee_cop_pred
    mass = float(np.asarray(trial_data["mass"])[0, 0])
    height = float(np.asarray(trial_data["height"])[0, 0])
    norm_factor = mass * height * 9.8067

    sources: Dict[str, List[float]] = {
        "Ground Truth": _left_kam_from_grf_percent_bwh(
            knee_cop_vectors=knee_cop_gt,
            grf=grf_gt,
            norm_factor=norm_factor,
        ).astype(float).tolist(),
        "LOSO Fine-Tuned": _left_kam_from_grf_percent_bwh(
            knee_cop_vectors=_align_1d_or_2d_length(
                np.asarray(
                    fine_tuned_predictions.get("predicted_knee_to_cop_vectors", knee_cop_pred),
                    dtype=np.float32,
                ),
                n_frames,
            ),
            grf=np.asarray(fine_tuned_predictions.get("grf"), dtype=np.float32),
            norm_factor=norm_factor,
        ).astype(float).tolist(),
    }
    original_pred = (
        (reference_outputs_eval.get("original_checkpoint_opencap_predinput", {}) or {})
        .get("predictions", {})
    )
    if isinstance(original_pred, Mapping) and original_pred.get("grf") is not None:
        sources["Original OpenCap"] = _left_kam_from_grf_percent_bwh(
            knee_cop_vectors=_align_1d_or_2d_length(
                np.asarray(original_pred.get("predicted_knee_to_cop_vectors", knee_cop_video), dtype=np.float32),
                n_frames,
            ),
            grf=np.asarray(original_pred.get("grf"), dtype=np.float32),
            norm_factor=norm_factor,
        ).astype(float).tolist()

    mask = infer_module._normalize_evaluation_mask(evaluation_mask, n_frames)
    return {
        "left_vgrf_n": grf_gt[:, 5].astype(float).tolist(),
        "right_vgrf_n": grf_gt[:, 2].astype(float).tolist(),
        "evaluation_mask": mask.astype(bool).tolist(),
        "sources": sources,
    }
...
def _plot_trunk_sway_kam_effects(
    *,
    ...
    styles = {
        ...
        "Original Motion Capture": {"color": "#264653", "linestyle": "-.", "dash": "dashdot"},
    }
...
def _build_trunk_sway_effect_summary(
    *,
    ...
    source_segments = {
        ...
        "Original Motion Capture": {"normal": [], "ts": []},
    }
    for condition_name, results in (("normal", normal_results), ("ts", trunk_sway_results)):
        for trial_result in results:
            ...
            mocap_pred, _mocap_gt = _collect_kam_segments_from_trial_result(
                trial_result,
                source_label="Original Motion Capture",
            )
            ...
            source_segments["Original Motion Capture"][condition_name].extend(mocap_pred)
...
def _direct_torque_left_kam_channel_index() -> int:
    names = list(DIRECT_TORQUE_NAMES or [])
    if "knee_adduction_l" in names:
        return int(names.index("knee_adduction_l"))
    return 11


def _direct_torque_left_leg_channel_indices(output_dim: int) -> List[int]:
    names = list(DIRECT_TORQUE_NAMES or [])
    indices = [
        idx
        for idx, name in enumerate(names[: int(output_dim)])
...
def _direct_torque_left_leg_mae_report(mae_report: Mapping[str, float]) -> Dict[str, float]:
    return {
        str(name): float(value)
        for name, value in dict(mae_report).items()
        if str(name).endswith("_l") and np.isfinite(float(value))
    }


def _select_interior_stances(
    phases: Sequence[Tuple[int, int]]
) -> List[Tuple[int, int]]:
    """Mirror infer.analyze_stance_phase_torques' stance selection.

    The standard (torque-informed) KAM path drops the first and last stance to
    keep interior steady-state strides (of two, keep the longer; a single stance
    is kept). Replicated here so the direct-torque trunk-sway analysis segments
    exactly the same strides as the standard path. See infer.py get_stance_phases.
    """
    phases = list(phases)
    if len(phases) > 2:
        return phases[1:-1]
    if len(phases) == 2:
        d0 = phases[0][1] - phases[0][0]
        d1 = phases[1][1] - phases[1][0]
        return [phases[0]] if d0 >= d1 else [phases[1]]
    return phases


def _direct_torque_stance_results_from_timeseries(
    *,
    left_vgrf_n: Sequence[float],
    gt_curve: Sequence[float],
    pred_curve: Sequence[float],
    evaluation_mask: Sequence[bool],
) -> Dict[str, Any]:
    kam_name = infer_module.LEFT_STANCE_KAM_DOF_NAME
    complete, _incomplete = _stance_intervals_from_vgrf(left_vgrf_n, mask=evaluation_mask)
    complete = _select_interior_stances(complete)
    pred_segments: List[np.ndarray] = []
    gt_segments: List[np.ndarray] = []
    for start, end in complete:
        pred_seg = _interp_segment_to_percent_stance(pred_curve, start, end)
        gt_seg = _interp_segment_to_percent_stance(gt_curve, start, end)
        if pred_seg is None or gt_seg is None:
            continue
        pred_segments.append(pred_seg)
        gt_segments.append(gt_seg)
    return {
        kam_name: {
            "pred": [seg.astype(float).tolist() for seg in pred_segments],
            "gt": [seg.astype(float).tolist() for seg in gt_segments],
            "n_stances": int(len(pred_segments)),
        }
    }


def _predict_direct_torque_full_trial_source(
    trial_info: Mapping[str, Any],
    *,
    model,
    params: Mapping[str, Any],
    ...
    """Predict full-trial direct torque for one input source, stitched over windows."""
    source_config = dict(config)
    source_config["loso_input_source"] = str(input_source)
    if str(input_source).lower() == "mocap":
        # Kinematics/Jacobian/auxiliary inputs come from MoCap, while GRF/GRM targets remain
        # from the processed force data, matching --MocapLoso behavior.
        source_config["loso_grf_grm_from_processed"] = True
    loader = _safe_trial_loader([trial_info], source_config, batch_size=1, shuffle=False)

    @jax.jit
    def _apply(model_params, x_batch, static_batch):
        return model.apply({"params": model_params}, x_batch, static_batch, train=False)

    pred_sum = None
    count = None
    for raw_batch in loader:
        input_norm = normalizers["input"].normalize(np.asarray(raw_batch["input"]))
        static_norm = normalizers["static"].normalize(np.asarray(raw_batch["static_context"]))
        pred_z = _apply(params, jnp.asarray(input_norm), jnp.asarray(static_norm))
        pred_pct = np.asarray(normalizers["direct_torque"].unnormalize(pred_z), dtype=np.float64)
        mask = np.asarray(raw_batch["supervision_mask"], dtype=np.float64)
        if mask.ndim == 2:
            mask = mask[..., None]
        starts = np.asarray(raw_batch["window_start_idx"], dtype=int)
        lengths = np.asarray(raw_batch["trial_length"], dtype=int)
        trial_len = int(lengths.max())
        output_dim = int(pred_pct.shape[-1])
        if pred_sum is None:
            pred_sum = np.zeros((trial_len, output_dim), dtype=np.float64)
        ...
        for b, start in enumerate(starts):
            for i in range(pred_pct.shape[1]):
                idx = int(start) + i
                if idx < 0 or idx >= pred_sum.shape[0] or float(mask[b, i, 0]) <= 0.0:
                    continue
                pred_sum[idx] += pred_pct[b, i]
                count[idx, 0] += 1.0
    if pred_sum is None or count is None:
        raise RuntimeError(f"No direct-torque windows produced for input_source={input_source}")
    valid_frames = count[:, 0] > 0
    pred_full = np.full_like(pred_sum, np.nan, dtype=np.float64)
    pred_full[valid_frames] = pred_sum[valid_frames] / count[valid_frames]
    return pred_full, valid_frames


def _evaluate_single_trial_direct_torque_for_ts(
    trial_info: Mapping[str, Any],
    *,
    model,
    ...
    if normalize_direct_batch is None:
        raise ImportError("Direct-torque TS evaluation requested but train_directTorque could not be imported.")

    trial_name = str(trial_info.get("trial_name", _trial_root_from_info(trial_info).name))
    trial_label = f"{trial_info.get('subject')}/{trial_name}"
    loader = _safe_trial_loader([trial_info], config, batch_size=1, shuffle=False)

    @jax.jit
    def _apply(params, x_batch, static_batch):
        return model.apply({"params": params}, x_batch, static_batch, train=False)
    ...
    kam_idx = _direct_torque_left_kam_channel_index()

    for raw_batch in loader:
        batch = normalize_direct_batch(raw_batch, normalizers)
        pred_z = _apply(final_params, batch["input"], batch["static_context"])
        original_z = _apply(original_params, batch["input"], batch["static_context"])
        pred_pct = np.asarray(normalizers["direct_torque"].unnormalize(pred_z), dtype=np.float64)
        original_pct = np.asarray(normalizers["direct_torque"].unnormalize(original_z), dtype=np.float64)
        target_pct = np.asarray(batch["direct_torque_target_raw"], dtype=np.float64)
        grf_ratio = np.asarray(raw_batch["grf"], dtype=np.float64)
        static_raw = np.asarray(batch.get("static_context_raw", raw_batch["static_context"]), dtype=np.float64)
        mass = static_raw[:, 1]
        grf_n = grf_ratio * mass[:, None, None] * 9.8067

        mask = np.asarray(batch["supervision_mask"], dtype=np.float64)
        if mask.ndim == 2:
            mask = mask[..., None]
        mask = mask * np.asarray(batch["direct_torque_finite_mask"], dtype=np.float64)
        starts = np.asarray(raw_batch["window_start_idx"], dtype=int)
        lengths = np.asarray(raw_batch["trial_length"], dtype=int)
        trial_len = int(lengths.max())
        output_dim = int(pred_pct.shape[-1])
        if pred_sum is None:
            pred_sum = np.zeros((trial_len, output_dim), dtype=np.float64)
            original_sum = np.zeros_like(pred_sum)
            target_sum = np.zeros_like(pred_sum)
            grf_sum = np.zeros((trial_len, 6), dtype=np.float64)
            count = np.zeros((trial_len, 1), dtype=np.float64)
        elif trial_len > pred_sum.shape[0]:
            grow = trial_len - pred_sum.shape[0]
            pred_sum = np.pad(pred_sum, ((0, grow), (0, 0)))
            original_sum = np.pad(original_sum, ((0, grow), (0, 0)))
            target_sum = np.pad(target_sum, ((0, grow), (0, 0)))
            grf_sum = np.pad(grf_sum, ((0, grow), (0, 0)))
            count = np.pad(count, ((0, grow), (0, 0)))
        ...
        for b, start in enumerate(starts):
            for i in range(pred_pct.shape[1]):
                ...
                if idx < 0 or idx >= pred_sum.shape[0] or float(mask[b, i, 0]) <= 0.0:
                    continue
                pred_sum[idx] += pred_pct[b, i]
                original_sum[idx] += original_pct[b, i]
                target_sum[idx] += target_pct[b, i]
                grf_sum[idx] += grf_n[b, i]
                count[idx, 0] += 1.0

        if np.any(mask > 0):
            all_abs_errors.append(np.abs(pred_pct - target_pct) * mask)

    if pred_sum is None or count is None:
        raise RuntimeError(f"No direct-torque windows produced for {trial_label}")
    valid_frames = count[:, 0] > 0
    pred_full = np.full_like(pred_sum, np.nan, dtype=np.float64)
    original_full = np.full_like(original_sum, np.nan, dtype=np.float64)
    original_mocap_full = None
    original_mocap_valid = None
    target_full = np.full_like(target_sum, np.nan, dtype=np.float64)
    grf_full = np.full_like(grf_sum, np.nan, dtype=np.float64)
    pred_full[valid_frames] = pred_sum[valid_frames] / count[valid_frames]
    original_full[valid_frames] = original_sum[valid_frames] / count[valid_frames]
    target_full[valid_frames] = target_sum[valid_frames] / count[valid_frames]
    grf_full[valid_frames] = grf_sum[valid_frames] / count[valid_frames]
    try:
        original_mocap_full, original_mocap_valid = _predict_direct_torque_full_trial_source(
            trial_info,
            model=model,
        ...
        original_mocap_full = _align_1d_or_2d_length(original_mocap_full, pred_full.shape[0])
        original_mocap_valid = infer_module._normalize_evaluation_mask(original_mocap_valid, pred_full.shape[0])
    except Exception as exc:
        print(f"   ℹ️ Original MoCap-input direct prediction skipped for {trial_label}: {exc}", flush=True)
        original_mocap_full = None
        original_mocap_valid = None

    left_vgrf = grf_full[:, 5]
    right_vgrf = grf_full[:, 2]
    gt_kam = target_full[:, kam_idx]
    pred_kam = pred_full[:, kam_idx]
    original_kam = original_full[:, kam_idx]
    original_mocap_kam = (
        original_mocap_full[:, kam_idx]
        if original_mocap_full is not None and original_mocap_full.shape[1] > kam_idx
        else None
    )
    stance_results = _direct_torque_stance_results_from_timeseries(
        left_vgrf_n=left_vgrf,
        gt_curve=gt_kam,
        pred_curve=pred_kam,
        evaluation_mask=valid_frames,
    )
    original_stance_results = _direct_torque_stance_results_from_timeseries(
        left_vgrf_n=left_vgrf,
        gt_curve=gt_kam,
        pred_curve=original_kam,
        evaluation_mask=valid_frames,
    )
    original_mocap_stance_results = None
    if original_mocap_kam is not None:
        original_mocap_stance_results = _direct_torque_stance_results_from_timeseries(
            left_vgrf_n=left_vgrf,
            gt_curve=gt_kam,
            pred_curve=original_mocap_kam,
            evaluation_mask=valid_frames & np.asarray(original_mocap_valid, dtype=bool),
        )

    err = pred_full[valid_frames] - target_full[valid_frames]
    per_channel_mae = np.nanmean(np.abs(err), axis=0) if err.size else np.full((pred_full.shape[1],), np.nan)
    original_err = original_full[valid_frames] - target_full[valid_frames]
    original_per_channel_mae = (
        np.nanmean(np.abs(original_err), axis=0)
        if original_err.size
        else np.full((original_full.shape[1],), np.nan)
    )
    if original_mocap_full is not None and original_mocap_valid is not None:
        mocap_eval_mask = valid_frames & np.asarray(original_mocap_valid, dtype=bool)
        original_mocap_err = original_mocap_full[mocap_eval_mask] - target_full[mocap_eval_mask]
        original_mocap_per_channel_mae = (
            np.nanmean(np.abs(original_mocap_err), axis=0)
            if original_mocap_err.size
            else np.full((original_mocap_full.shape[1],), np.nan)
        )
    else:
        mocap_eval_mask = np.zeros_like(valid_frames, dtype=bool)
        original_mocap_err = np.empty((0, pred_full.shape[1]), dtype=np.float64)
        original_mocap_per_channel_mae = np.full((pred_full.shape[1],), np.nan)
    channel_names = list(DIRECT_TORQUE_NAMES or [f"direct_torque_{idx}" for idx in range(pred_full.shape[1])])
    mae_report_all = {
        str(name): float(value)
        for name, value in zip(channel_names, per_channel_mae)
        if np.isfinite(value)
    }
    original_mae_report_all = {
        str(name): float(value)
        for name, value in zip(channel_names, original_per_channel_mae)
        if np.isfinite(value)
    }
    original_mocap_mae_report_all = {
        str(name): float(value)
        for name, value in zip(channel_names, original_mocap_per_channel_mae)
        if np.isfinite(value)
    }
    mae_report = _direct_torque_left_leg_mae_report(mae_report_all)
    original_mae_report = _direct_torque_left_leg_mae_report(original_mae_report_all)
    original_mocap_mae_report = _direct_torque_left_leg_mae_report(original_mocap_mae_report_all)
    left_channel_indices = _direct_torque_left_leg_channel_indices(pred_full.shape[1])
    err_left = err[:, left_channel_indices] if err.size and left_channel_indices else err
    metrics = {
        "trial_name": trial_label,
        "n_eval_frames": int(np.sum(valid_frames)),
        "direct_torque_mae_bwh": float(np.nanmean(np.abs(err_left))) if err_left.size else float("nan"),
        "direct_torque_rmse_bwh": float(np.sqrt(np.nanmean(np.square(err_left)))) if err_left.size else float("nan"),
        "left_kam_mae_bwh": float(np.nanmean(np.abs(pred_kam[valid_frames] - gt_kam[valid_frames]))),
        "left_kam_original_mae_bwh": float(np.nanmean(np.abs(original_kam[valid_frames] - gt_kam[valid_frames]))),
        "left_kam_original_mocap_input_mae_bwh": (
            float(np.nanmean(np.abs(original_mocap_kam[mocap_eval_mask] - gt_kam[mocap_eval_mask])))
            if original_mocap_kam is not None and np.any(mocap_eval_mask)
            else float("nan")
        ),
    }
    metrics.update({f"mae_{name}": value for name, value in mae_report.items()})

    safe_name = trial_label.replace("/", "__")
    trial_dir = output_root / safe_name
    trial_dir.mkdir(parents=True, exist_ok=True)
    np.save(trial_dir / "direct_torque_pred_percent_bwh.npy", pred_full.astype(np.float32))
    np.save(trial_dir / "direct_torque_original_percent_bwh.npy", original_full.astype(np.float32))
    if original_mocap_full is not None:
        np.save(trial_dir / "direct_torque_original_mocap_input_percent_bwh.npy", original_mocap_full.astype(np.float32))
    np.save(trial_dir / "direct_torque_gt_percent_bwh.npy", target_full.astype(np.float32))
    np.save(trial_dir / "evaluation_mask.npy", valid_frames)
    with (trial_dir / "metrics.json").open("w", encoding="utf-8") as handle:
        json.dump(metrics, handle, indent=2)
    with (trial_dir / "mae_report.json").open("w", encoding="utf-8") as handle:
        json.dump(mae_report, handle, indent=2)

    try:
        fig, axes = infer_module.plt.subplots(7, 2, figsize=(14, 18), sharex=True)
        axes = np.asarray(axes).reshape(-1)
        x = np.arange(pred_full.shape[0])
        for idx, name in enumerate(channel_names[: len(axes)]):
            ax = axes[idx]
            ax.plot(x, target_full[:, idx], label="GT", linewidth=1.2)
            ax.plot(x, pred_full[:, idx], label="LOSO Fine-Tuned", linewidth=1.0)
            ax.plot(x, original_full[:, idx], label="Original OpenCap", linewidth=0.9, linestyle="--")
            if original_mocap_full is not None:
                ax.plot(x, original_mocap_full[:, idx], label="Original MoCap Input", linewidth=0.9, linestyle=":")
            ax.set_title(str(name))
            ax.set_ylabel("%BW*H")
            ax.grid(True, alpha=0.25)
        axes[0].legend(fontsize=8)
        axes[-1].set_xlabel("Frame")
        fig.suptitle(f"Direct Torque Infer-Style Evaluation: {trial_label}", fontsize=14, fontweight="bold")
        fig.tight_layout(rect=[0, 0, 1, 0.98])
        fig.savefig(trial_dir / "direct_torque_timeseries.png", dpi=150)
        infer_module.plt.close(fig)
    except Exception as exc:
        print(f"   ⚠️ Direct torque timeseries plot failed for {trial_label}: {exc}", flush=True)

    return {
        "trial_name": trial_label,
        "trial_output_dir": str(trial_dir),
        "evaluation_frame_count": int(np.sum(valid_frames)),
        "inference_time_ms": float("nan"),
        "metrics": metrics,
        "mae_report": mae_report,
        "reference_mae_reports": {
            "Original OpenCap PredInput": original_mae_report,
            "Original Motion Capture": original_mocap_mae_report,
        },
        "stance_results": stance_results,
        "reference_stance_results": {
            "Original OpenCap PredInput": original_stance_results,
            **({"Original Motion Capture": original_mocap_stance_results} if original_mocap_stance_results else {}),
        },
        "left_kam_timeseries": {
            "left_vgrf_n": np.nan_to_num(left_vgrf, nan=0.0).astype(float).tolist(),
            "right_vgrf_n": np.nan_to_num(right_vgrf, nan=0.0).astype(float).tolist(),
            "evaluation_mask": valid_frames.astype(bool).tolist(),
            "sources": {
                "Ground Truth": gt_kam.astype(float).tolist(),
                "LOSO Fine-Tuned": pred_kam.astype(float).tolist(),
                "Original OpenCap": original_kam.astype(float).tolist(),
                **(
                    {"Original Motion Capture": original_mocap_kam.astype(float).tolist()}
                    if original_mocap_kam is not None
                    else {}
                ),
            },
...
def _run_trunk_sway_effect_evaluation(
    ...
    *,
    ...
    model,
    params: Mapping[str, Any],
    original_params: Optional[Mapping[str, Any]] = None,
    normalizers: Mapping[str, Any],
    config: Mapping[str, Any],
) -> Dict[str, Any]:
    held_out_subject = str(fold["held_out_subject"])
    ...
    if _is_direct_torque_config(config):
            ...
            for trial_info in condition_trials:
                ...
                try:
                    result_bucket.append(
                        _evaluate_single_trial_direct_torque_for_ts(
                            trial_info,
                            model=model,
                            final_params=params,
                            original_params=original_params,
                    ...
                    print(
                        f"   ⚠️ Direct trunk-sway effect evaluation failed for {condition_name} {trial_name}: {exc}",
                        flush=True,
                    )
                    failures.append(
                        {
        ...
        payload.update(
            {
                ...
                "kam_trial_timeseries": {
                    "normal": [
                        {
                            "trial_name": str(result.get("trial_name")),
                            **dict(result.get("left_kam_timeseries") or {}),
                        }
                        for result in normal_results
                        if isinstance(result.get("left_kam_timeseries"), Mapping)
                    ],
                    "trunk_sway": [
                        {
                            "trial_name": str(result.get("trial_name")),
...
_COHORT_KAM_SOURCES = ("Ground Truth", "LOSO Fine-Tuned", "Original OpenCap", "Original Motion Capture")
_COHORT_KAM_CURVE_KEYS = ("normal_curve", "trunk_sway_curve", "effect_curve")
_COHORT_KAM_SCALAR_KEYS = ("normal_mean", "trunk_sway_mean", "mean_effect", "peak_abs_effect", "cohens_d")


def _cohort_finite_curve(value: Any) -> Optional[np.ndarray]:
    """Return a length-101 finite curve as float64, else None."""
    if value is None:
        return None
    arr = np.asarray(value, dtype=np.float64).reshape(-1)
    if arr.size != 101 or not np.all(np.isfinite(arr)):
        return None
    return arr


def _cohort_pearson_r(a: np.ndarray, b: np.ndarray) -> float:
    if a.size < 2 or np.std(a) < 1e-12 or np.std(b) < 1e-12:
        return float("nan")
    return float(np.corrcoef(a, b)[0, 1])


def _plot_trunk_sway_kam_cohort(
    payload: Mapping[str, Any],
    output_dir: Path,
    *,
    per_subject: Mapping[str, Mapping[str, Mapping[str, Any]]],
    paired_subjects: Sequence[str],
) -> None:
    """Cohort KAM plots: mean±std by condition, GT-vs-LOSO effect scatter, effect bars."""
    plt = infer_module.plt
    stance = np.linspace(0.0, 100.0, 101)
    styles = {
        "Ground Truth": "#2E86AB",
        "LOSO Fine-Tuned": "#E94F37",
        "Original OpenCap": "#1B9E77",
        "Original Motion Capture": "#264653",
    }
    sources = payload.get("sources", {})

    # Fig A — cohort KAM by condition (mean solid = normal, dashed = trunk sway, ±std shaded).
    fig, ax = plt.subplots(figsize=(9, 5))
    for src, s in sources.items():
        color = styles.get(src, "#444444")
        nm = s.get("normal_curve_mean")
        ns = s.get("normal_curve_std")
        tm = s.get("trunk_sway_curve_mean")
        if nm is not None:
            nm = np.asarray(nm)
            ax.plot(stance, nm, color=color, linestyle="-", linewidth=2.0, label=f"{src} normal")
            if ns is not None:
                ns = np.asarray(ns)
                ax.fill_between(stance, nm - ns, nm + ns, color=color, alpha=0.12)
        if tm is not None:
            tm = np.asarray(tm)
            ax.plot(stance, tm, color=color, linestyle="--", linewidth=2.0, label=f"{src} trunk sway")
    ax.set_title(f"Cohort knee adduction moment by condition (n={payload.get('n_subjects', 0)} subjects)")
    ax.set_xlabel("% left stance")
    ax.set_ylabel("KAM (% BW*H)")
    ax.grid(alpha=0.25)
    ax.legend(frameon=False, fontsize=8)
    fig.tight_layout()
    fig.savefig(output_dir / "cohort_kam_by_condition.png", dpi=200)
    plt.close(fig)

    # Fig B — per-subject TS-effect agreement: GT (x) vs LOSO (y).
    gt = per_subject.get("Ground Truth", {})
    loso = per_subject.get("LOSO Fine-Tuned", {})
    xs, ys = [], []
    for subj in paired_subjects:
        if "mean_effect" in gt.get(subj, {}) and "mean_effect" in loso.get(subj, {}):
            xs.append(gt[subj]["mean_effect"])
            ys.append(loso[subj]["mean_effect"])
    fig, ax = plt.subplots(figsize=(6, 6))
    if xs:
        xs_a = np.asarray(xs)
        ys_a = np.asarray(ys)
        ax.scatter(xs_a, ys_a, color="#E94F37", zorder=3)
        lo = float(min(xs_a.min(), ys_a.min()))
        hi = float(max(xs_a.max(), ys_a.max()))
        ax.plot([lo, hi], [lo, hi], color="#888888", linestyle=":", label="identity")
        agree = payload.get("agreement_LOSO_vs_GT", {})
        r = agree.get("mean_effect_pearson_r")
        mae = agree.get("mean_effect_mae")
        if r == r and mae == mae:  # not nan
            ax.set_title(f"TS effect agreement (r={r:.2f}, MAE={mae:.3f} %BW*H)")
        else:
            ax.set_title("TS effect agreement")
    ax.set_xlabel("GT mean TS effect (% BW*H)")
    ax.set_ylabel("LOSO mean TS effect (% BW*H)")
    ax.grid(alpha=0.25)
    fig.tight_layout()
    fig.savefig(output_dir / "cohort_kam_effect_agreement.png", dpi=200)
    plt.close(fig)

    # Fig C — cohort mean TS effect per source (mean ± std).
    labels = list(sources.keys())
    if labels:
        means = [sources[l].get("mean_effect_mean", float("nan")) for l in labels]
        stds = [sources[l].get("mean_effect_std", 0.0) for l in labels]
        colors = [styles.get(l, "#444444") for l in labels]
        fig, ax = plt.subplots(figsize=(7, 5))
        ax.bar(labels, means, yerr=stds, color=colors, capsize=4)
        ax.axhline(0.0, color="#888888", linewidth=1.0)
        ax.set_title("Cohort mean trunk-sway effect")
        ax.set_ylabel("Delta KAM (% BW*H)")
        ax.tick_params(axis="x", rotation=20)
        fig.tight_layout()
        fig.savefig(output_dir / "cohort_kam_effect_bars.png", dpi=200)
        plt.close(fig)


def _stance_intervals_from_vgrf(
    vgrf: Sequence[float],
    *,
    ...
    """Return complete stance intervals and starts without a complete end."""
    arr = np.asarray(vgrf, dtype=np.float64).reshape(-1)
    if threshold_n is None:
        threshold_n = float(getattr(infer_module, "LEFT_STANCE_THRESHOLD_N", 20.0))
    if mask is None:
        valid = np.ones(arr.shape[0], dtype=bool)
    else:
        valid = infer_module._normalize_evaluation_mask(np.asarray(mask, dtype=bool), arr.shape[0])
    stance = (np.abs(arr) > float(threshold_n)) & valid
    if stance.size == 0:
        return [], []

    diff = np.diff(stance.astype(np.int8), prepend=0, append=0)
    starts = np.where(diff == 1)[0].astype(int).tolist()
    ends = np.where(diff == -1)[0].astype(int).tolist()
    ...
    for start in starts:
        ...
        if end_candidates:
            ...
            if end - start >= int(min_duration_frames):
                complete.append((int(start), end))
            elif stance[-1] and start <= np.where(stance)[0][-1]:
                incomplete.append(int(start))
        else:
            incomplete.append(int(start))
    return complete, incomplete


def _interp_segment_to_percent_stance(curve: Sequence[float], start: int, end: int) -> Optional[np.ndarray]:
    arr = np.asarray(curve, dtype=np.float64).reshape(-1)
    start = max(0, int(start))
    end = min(int(end), arr.shape[0])
    if end - start < 2:
        return None
    ...
    if not np.all(np.isfinite(segment)):
        ...
        if np.count_nonzero(finite) < 2:
            return None
        segment = np.interp(x, x[finite], segment[finite])
    x_old = np.linspace(0.0, 100.0, segment.size, dtype=np.float64)
    x_new = np.linspace(0.0, 100.0, 101, dtype=np.float64)
    return np.interp(x_new, x_old, segment).astype(np.float32)


def _iter_kam_trial_payloads(
    fold_results: Sequence[Mapping[str, Any]],
) -> Iterable[Tuple[str, str, str, Mapping[str, Any]]]:
    for fold_result in fold_results:
        payload = fold_result.get("trunk_sway_effect_eval") if isinstance(fold_result, Mapping) else None
        if not isinstance(payload, Mapping) or payload.get("skipped"):
            continue
        trial_payloads = payload.get("kam_trial_timeseries")
        if not isinstance(trial_payloads, Mapping):
            continue
        held_out = str(payload.get("held_out_subject", ""))
        for condition in ("normal", "trunk_sway"):
            entries = trial_payloads.get(condition) or []
            if not isinstance(entries, Sequence):
                continue
            for entry in entries:
                if not isinstance(entry, Mapping):
                    continue
                yield held_out, condition, str(entry.get("trial_name", "")), entry


def _aggregate_all_stance_kam_segments_across_folds(
    fold_results: Sequence[Mapping[str, Any]],
    output_root: Path,
) -> Optional[Dict[str, Any]]:
    """Segment every normal/TS KAM curve by left stance and plot cohort condition curves."""
    entries = list(_iter_kam_trial_payloads(fold_results))
    if not entries:
        return None

    complete_durations: List[int] = []
    right_durations_by_trial: Dict[Tuple[str, str, str], List[int]] = {}
    for held_out, condition, trial_name, entry in entries:
        mask = entry.get("evaluation_mask")
        left_complete, _left_incomplete = _stance_intervals_from_vgrf(entry.get("left_vgrf_n", []), mask=mask)
        right_complete, _right_incomplete = _stance_intervals_from_vgrf(entry.get("right_vgrf_n", []), mask=mask)
        durations = [end - start for start, end in left_complete + right_complete if end > start]
        complete_durations.extend(durations)
        right_durations_by_trial[(held_out, condition, trial_name)] = [
            end - start for start, end in right_complete if end > start
        ]

    avg_duration = int(round(float(np.mean(complete_durations)))) if complete_durations else 60
    avg_duration = max(6, avg_duration)
    segments: Dict[str, Dict[str, Dict[str, List[np.ndarray]]]] = {
        "normal": {src: {"segments": []} for src in _COHORT_KAM_SOURCES},
        "trunk_sway": {src: {"segments": []} for src in _COHORT_KAM_SOURCES},
    }
    segment_meta: List[Dict[str, Any]] = []

    for held_out, condition, trial_name, entry in entries:
        mask = entry.get("evaluation_mask")
        left_complete, left_incomplete = _stance_intervals_from_vgrf(entry.get("left_vgrf_n", []), mask=mask)
        right_durations = right_durations_by_trial.get((held_out, condition, trial_name), [])
        fallback_duration = int(round(float(np.mean(right_durations)))) if right_durations else avg_duration
        fallback_duration = max(6, fallback_duration)
        intervals: List[Tuple[int, int, str]] = [(s, e, "complete_left") for s, e in left_complete]
        n_frames = len(np.asarray(entry.get("left_vgrf_n", [])).reshape(-1))
        for start in left_incomplete:
            end = min(n_frames, int(start) + int(fallback_duration))
            if end - int(start) >= 6:
                source = "right_foot_duration" if right_durations else "global_average_duration"
                intervals.append((int(start), end, source))

        if not intervals:
            continue

        source_curves = entry.get("sources") if isinstance(entry.get("sources"), Mapping) else {}
        for start, end, interval_source in intervals:
            any_source_added = False
            for source_label in _COHORT_KAM_SOURCES:
                curve = source_curves.get(source_label) if isinstance(source_curves, Mapping) else None
                if curve is None:
                    continue
                seg = _interp_segment_to_percent_stance(curve, start, end)
                if seg is None:
                    continue
                segments[condition][source_label]["segments"].append(seg)
                any_source_added = True
            if any_source_added:
                segment_meta.append(
                    {
                        ...
                        "duration_source": interval_source,
                    }
                )

    source_out: Dict[str, Dict[str, Any]] = {"normal": {}, "trunk_sway": {}}
    for condition, source_map in segments.items():
        for source_label, payload in source_map.items():
            segs = payload["segments"]
            if not segs:
                continue
            stack = np.vstack(segs)
            source_out[condition][source_label] = {
                "n_segments": int(stack.shape[0]),
                "mean_curve": np.nanmean(stack, axis=0).tolist(),
                "std_curve": np.nanstd(stack, axis=0).tolist(),
            }

    if not any(source_out[condition] for condition in source_out):
        return None

    out_dir = output_root / "trunk_sway_cohort_summary"
    out_dir.mkdir(parents=True, exist_ok=True)
    payload_out = {
        "kam_dof": infer_module.LEFT_STANCE_KAM_DOF_NAME,
        "units": "percent_BW_times_height",
        "segmentation": {
            "primary": "left vertical GRF stance",
            "incomplete_left_fallback_order": [
                "mean complete right-foot stance duration within the same trial",
                "global mean complete stance duration across all normal and trunk-sway trials",
            ],
            "global_average_duration_frames": int(avg_duration),
            "n_complete_duration_samples": int(len(complete_durations)),
        },
        "conditions": source_out,
        "segments": segment_meta,
    }
    _save_json(out_dir / "all_left_stance_kam_segments_summary.json", payload_out)
    ...
    for condition, source_map in source_out.items():
        if not source_map:
            continue
        fig, ax = plt.subplots(figsize=(9, 5))
        for source_label in _COHORT_KAM_SOURCES:
            summary = source_map.get(source_label)
            if not isinstance(summary, Mapping):
                continue
            mean = np.asarray(summary["mean_curve"], dtype=np.float64)
            std = np.asarray(summary["std_curve"], dtype=np.float64)
            color = styles.get(source_label, "#444444")
            n = int(summary.get("n_segments", 0))
            ax.plot(stance, mean, color=color, linewidth=2.3, label=f"{source_label} (n={n})")
            ax.fill_between(stance, mean - std, mean + std, color=color, alpha=0.12)
        ax.set_title(f"{condition_titles.get(condition, condition)} KAM by left stance")
        ax.set_xlabel("% left stance")
        ax.set_ylabel("KAM (% BW*H)")
        ax.grid(alpha=0.25)
        ax.legend(frameon=False, fontsize=8)
        fig.tight_layout()
        fig.savefig(out_dir / f"all_left_stance_kam_{condition}.png", dpi=200)
        plt.close(fig)
    return payload_out


def _aggregate_trunk_sway_kam_across_folds(
    fold_results: Sequence[Mapping[str, Any]],
    output_root: Path,
) -> Optional[Dict[str, Any]]:
    """Combine per-fold trunk-sway KAM summaries into one cohort-level summary.

    Reads each fold's ``fold_result['trunk_sway_effect_eval']`` (written by
    ``_run_trunk_sway_effect_evaluation``). For each KAM source (Ground Truth,
    LOSO Fine-Tuned, Original OpenCap) it reports cohort mean/std of the scalar TS
    effects and the length-101 curves, plus LOSO-vs-GT agreement (per-subject TS
    effect correlation / MAE / bias, effect-curve RMSE) and per-condition KAM
    prediction accuracy vs GT. Returns None (and writes nothing) when no fold has a
    usable trunk-sway summary.
    """
    usable: List[Mapping[str, Any]] = []
    for fold_result in fold_results:
        if not isinstance(fold_result, Mapping):
            continue
        payload = fold_result.get("trunk_sway_effect_eval")
        if not isinstance(payload, Mapping):
            continue
        if not payload.get("enabled") or payload.get("skipped"):
            continue
        if not isinstance(payload.get("sources"), Mapping) or not payload["sources"]:
            continue
    ...
    subjects = [str(p.get("held_out_subject")) for p in usable]

    # Per source: gather per-subject scalars + curves, then cohort-reduce.
    per_subject: Dict[str, Dict[str, Dict[str, Any]]] = {src: {} for src in _COHORT_KAM_SOURCES}
    source_out: Dict[str, Any] = {}
    for src in _COHORT_KAM_SOURCES:
        scalars: Dict[str, List[float]] = {k: [] for k in _COHORT_KAM_SCALAR_KEYS}
        curves: Dict[str, List[np.ndarray]] = {k: [] for k in _COHORT_KAM_CURVE_KEYS}
        for payload in usable:
            subj = str(payload.get("held_out_subject"))
            source_summary = payload["sources"].get(src)
            if not isinstance(source_summary, Mapping):
                continue
            entry: Dict[str, Any] = {}
            for key in _COHORT_KAM_SCALAR_KEYS:
                val = source_summary.get(key)
                if val is not None and np.isfinite(val):
                    scalars[key].append(float(val))
                    entry[key] = float(val)
            for key in _COHORT_KAM_CURVE_KEYS:
                curve = _cohort_finite_curve(source_summary.get(key))
                if curve is not None:
                    curves[key].append(curve)
                    entry[key] = curve
            if entry:
                per_subject[src][subj] = entry
        n_src = max((len(v) for v in scalars.values()), default=0)
        if n_src == 0 and not any(curves.values()):
            continue
        src_summary: Dict[str, Any] = {"n_subjects": len(per_subject[src])}
        for key in _COHORT_KAM_SCALAR_KEYS:
            vals = scalars[key]
            src_summary[f"{key}_mean"] = float(np.mean(vals)) if vals else float("nan")
            src_summary[f"{key}_std"] = (
                float(np.std(vals, ddof=1)) if len(vals) >= 2 else (0.0 if vals else float("nan"))
            )
        for key in _COHORT_KAM_CURVE_KEYS:
            if curves[key]:
                stack = np.vstack(curves[key])
                src_summary[f"{key}_mean"] = np.nanmean(stack, axis=0).tolist()
                src_summary[f"{key}_std"] = np.nanstd(stack, axis=0).tolist()
        source_out[src] = src_summary

    # LOSO-vs-GT agreement (paired by subject).
    gt = per_subject.get("Ground Truth", {})
    loso = per_subject.get("LOSO Fine-Tuned", {})
    paired_subjects = [s for s in subjects if s in gt and s in loso]

    agreement: Dict[str, Any] = {"n_paired_subjects": len(paired_subjects)}
    for scalar in ("mean_effect", "normal_mean", "trunk_sway_mean"):
        pairs = [
            (gt[s][scalar], loso[s][scalar])
            for s in paired_subjects
            if scalar in gt[s] and scalar in loso[s]
        ]
        if pairs:
            gt_arr = np.asarray([p[0] for p in pairs], dtype=np.float64)
            loso_arr = np.asarray([p[1] for p in pairs], dtype=np.float64)
            agreement[f"{scalar}_pearson_r"] = _cohort_pearson_r(gt_arr, loso_arr)
            agreement[f"{scalar}_mae"] = float(np.mean(np.abs(loso_arr - gt_arr)))
            agreement[f"{scalar}_bias"] = float(np.mean(loso_arr - gt_arr))  # LOSO - GT
        else:
            agreement[f"{scalar}_pearson_r"] = float("nan")
            agreement[f"{scalar}_mae"] = float("nan")
            agreement[f"{scalar}_bias"] = float("nan")

    curve_rmses = []
    for s in paired_subjects:
        gt_curve = gt[s].get("effect_curve")
        loso_curve = loso[s].get("effect_curve")
        if gt_curve is not None and loso_curve is not None:
            curve_rmses.append(float(np.sqrt(np.mean((np.asarray(loso_curve) - np.asarray(gt_curve)) ** 2))))
    agreement["effect_curve_rmse"] = float(np.mean(curve_rmses)) if curve_rmses else float("nan")

    # Per-condition KAM prediction accuracy (LOSO curve vs GT curve), cohort mean±std.
    kam_pred_acc: Dict[str, Any] = {}
    for cond, curve_key in (("normal", "normal_curve"), ("trunk_sway", "trunk_sway_curve")):
        maes = []
        for s in paired_subjects:
            gt_curve = gt[s].get(curve_key)
            loso_curve = loso[s].get(curve_key)
            if gt_curve is not None and loso_curve is not None:
                maes.append(float(np.mean(np.abs(np.asarray(loso_curve) - np.asarray(gt_curve)))))
        kam_pred_acc[f"{cond}_curve_mae_mean"] = float(np.mean(maes)) if maes else float("nan")
        kam_pred_acc[f"{cond}_curve_mae_std"] = (
            float(np.std(maes, ddof=1)) if len(maes) >= 2 else (0.0 if maes else float("nan"))
        )
    ...
    payload_out = {
        ...
        "subjects": subjects,
        "sources": source_out,
        "agreement_LOSO_vs_GT": agreement,
        "kam_pred_accuracy_vs_GT": kam_pred_acc,
    }
    out_dir = output_root / "trunk_sway_cohort_summary"
    out_dir.mkdir(parents=True, exist_ok=True)
    _save_json(out_dir / "cohort_kam_trunk_sway_summary.json", payload_out)
    try:
        _plot_trunk_sway_kam_cohort(
            payload_out,
            out_dir,
            per_subject=per_subject,
            paired_subjects=paired_subjects,
        )
    except Exception as exc:  # never fail the LOSO run over a plot
        print(f"   ⚠️ Cohort KAM plotting failed: {exc}", flush=True)
    return payload_out
...
def _run_train_epoch(
    ...
    *,
    ...
    if direct_torque:
        if normalize_direct_batch is None:
            raise ImportError("Direct-torque LOSO requested but train_directTorque could not be imported.")
        sums = {"total_loss": 0.0, "loss": 0.0, "mae_bwh": 0.0, "rmse_bwh": 0.0}
        steps = 0
        for batch in train_loader:
            batch_norm = normalize_direct_batch(batch, normalizers)
            rng, dropout_rng = jax.random.split(rng)
            state, step_metrics = train_step(state, batch_norm, dropout_rng)
            sums["loss"] += float(step_metrics["loss"])
            sums["total_loss"] += float(step_metrics["loss"])
            sums["mae_bwh"] += float(step_metrics["mae_bwh"])
            sums["rmse_bwh"] += float(step_metrics["rmse_bwh"])
            steps += 1
        if steps == 0:
            raise RuntimeError("Training loader yielded zero batches.")
        return state, {key: value / steps for key, value in sums.items()}, rng
    ...
    metrics_sum = {
        ...
        "contact_loss": 0.0,
        "torque_loss": 0.0,
        "torque_knee_adduction_loss": 0.0,
        "torque_cop_effect_loss": 0.0,
        "torque_grf_effect_loss": 0.0,
        "torque_cop_grad_rms": 0.0,
        "torque_grf_grad_rms": 0.0,
        "torque_knee_adduction_cop_grad_rms": 0.0,
        "torque_knee_adduction_grf_grad_rms": 0.0,
        "torque_knee_adduction_to_torque_cop_grad_ratio": 0.0,
        "torque_knee_adduction_to_torque_grf_grad_ratio": 0.0,
    ...
    *,
    ...
    if _is_direct_torque_config(config):
        if normalize_direct_batch is None:
            raise ImportError("Direct-torque LOSO requested but train_directTorque could not be imported.")
        sums = {"total_loss": 0.0, "loss": 0.0, "mae_bwh": 0.0, "rmse_bwh": 0.0}
        left_abs_err_sum = 0.0
        left_sq_err_sum = 0.0
        left_count = 0.0
        steps = 0
        for batch in loader:
            batch_norm = normalize_direct_batch(batch, normalizers)
            step_metrics, step_pred = eval_step(state, batch_norm)
            sums["loss"] += float(step_metrics["loss"])
            sums["total_loss"] += float(step_metrics["loss"])
            sums["mae_bwh"] += float(step_metrics["mae_bwh"])
            sums["rmse_bwh"] += float(step_metrics["rmse_bwh"])
            try:
                pred_pct = np.asarray(normalizers["direct_torque"].unnormalize(step_pred), dtype=np.float64)
                target_pct = np.asarray(batch_norm["direct_torque_target_raw"], dtype=np.float64)
                finite_mask = np.asarray(batch_norm["direct_torque_finite_mask"], dtype=np.float64)
                supervision_mask = np.asarray(batch_norm["supervision_mask"], dtype=np.float64)
                if supervision_mask.ndim == 2:
                    supervision_mask = supervision_mask[..., None]
                while supervision_mask.ndim < pred_pct.ndim:
                    supervision_mask = supervision_mask[..., None]
                while finite_mask.ndim < pred_pct.ndim:
                    finite_mask = finite_mask[..., None]
                left_indices = _direct_torque_left_leg_channel_indices(pred_pct.shape[-1])
                if left_indices:
                    err_left = pred_pct[..., left_indices] - target_pct[..., left_indices]
                    mask_left = supervision_mask * finite_mask[..., left_indices]
                    left_abs_err_sum += float(np.nansum(np.abs(err_left) * mask_left))
                    left_sq_err_sum += float(np.nansum(np.square(err_left) * mask_left))
                    left_count += float(np.nansum(mask_left))
            except Exception:
                pass
            steps += 1
        if steps == 0:
        ...
        if left_count > 0:
            metrics["direct_torque_mae_bwh"] = float(left_abs_err_sum / left_count)
            metrics["direct_torque_rmse_bwh"] = float(np.sqrt(left_sq_err_sum / left_count))
            metrics["mae_bwh"] = metrics["direct_torque_mae_bwh"]
            metrics["rmse_bwh"] = metrics["direct_torque_rmse_bwh"]
        else:
            metrics["direct_torque_mae_bwh"] = metrics["mae_bwh"]
            metrics["direct_torque_rmse_bwh"] = metrics["rmse_bwh"]
        return {"metrics": metrics, "mae_report": {}, "stance_results": {}}
    ...
    for batch in loader:
        ...
        try:
            ...
            if knee_to_cop_vectors is not None:
                ...
                if _eval_gt_kin:
                    ankle_pos_eval = batch.get("gt_ankle_pos", batch.get("ankle_pos"))
                    knee_pos_eval = batch.get("gt_knee_pos", batch.get("knee_pos"))
                    rot_kam_eval = _rot_eval
                else:
                    ankle_pos_eval = batch.get("ankle_pos")
                    knee_pos_eval = batch.get("knee_pos")
                    rot_kam_eval = batch["rot_w_to_ga"]
                if ankle_pos_eval is None or knee_pos_eval is None:
                    raise ValueError("ankle_pos/knee_pos are required for predicted-COP KAM evaluation.")
                pred_knee_to_cop = np.array(
                    train_module.compute_predicted_knee_to_cop_vectors(
                        jnp.array(cop_pred_phys),
                        jnp.array(ankle_pos_eval),
                        jnp.array(knee_pos_eval),
                        jnp.array(rot_kam_eval),
                    )
                )
                z_vec_l = knee_to_cop[:, :, 5]
                y_vec_l = knee_to_cop[:, :, 4]
                pred_z_vec_l = pred_knee_to_cop[:, :, 5]
                pred_y_vec_l = pred_knee_to_cop[:, :, 4]
                kam_pred = pred_z_vec_l * grf_pred_phys[:, :, 4] - pred_y_vec_l * grf_pred_phys[:, :, 5]
...
def _run_inner_epoch_validation_fold(
    *,
    ...
    with log_path.open("w", encoding="utf-8") as log_handle:
        ...
        if _is_direct_torque_config(fold_config):
            if make_direct_train_step is None or make_direct_eval_step is None:
                raise ImportError("Direct-torque LOSO requested but train_directTorque could not be imported.")
            train_step = make_direct_train_step(
                normalizers,
                str(fold_config.get("robust_loss", "huber")),
                float(fold_config.get("huber_delta", 1.0)),
            )
            eval_step = make_direct_eval_step(
                normalizers,
                str(fold_config.get("robust_loss", "huber")),
                float(fold_config.get("huber_delta", 1.0)),
            )
            ...
            eval_step = train_module.make_eval_step(
                ...
                cop_mask=bool(fold_config["cop_mask"]),
                use_grf_norm_cop=bool(fold_config.get("use_grf_norm_cop", False)),
                use_gt_jacob_and_rot=bool(fold_config.get("use_gt_jacob_and_rot_for_eval", False)),
            )
            ...
            if _configure_kam_loss_weight_for_mode(
                loss_weights,
                fold_config,
                log_prefix=f"[{test_subject}/inner_{inner_val_subject}]",
            ):
                _calibrate_kam_loss_weight_first_batch(
                    state=state,
        ...
        for epoch in range(1, int(max_epochs) + 1):
            ...
            state, train_losses, rng = _run_train_epoch(
                ...
                rng=rng,
                epoch=epoch,
                direct_torque=_is_direct_torque_config(fold_config),
            )
            _log(
    ...
    *,
    ...
    if _is_direct_torque_config(fold_config):
        ...
        if _configure_kam_loss_weight_for_mode(
            loss_weights,
            fold_config,
            log_prefix=f"[{fold['held_out_subject']}]",
        ):
            _calibrate_kam_loss_weight_first_batch(
                state=state,
                train_loader=train_loader,
    ...
    held_out_infer_style = None
    if _is_direct_torque_config(fold_config):
        print(
            f"[{fold['held_out_subject']}] direct-torque checkpoint detected; "
            "running direct-torque infer-style evaluation.",
            flush=True,
        )
        held_out_infer_style = _run_direct_torque_infer_style_evaluation(
            fold,
            fold_dir=fold_dir,
            model=model,
            params=final_state.params,
            original_params=checkpoint["params"],
            normalizers=normalizers,
            config=fold_config,
        )
    else:
    ...
    fold_checkpoint_payload = {
        ...
        "normalizers": normalizers,
        "model_structure": str(fold_config.get("model_structure", "cop_grf_moments")),
        "model_type": str(fold_config.get("model_type", fold_config.get("model_structure", "cop_grf_moments"))),
        "held_out_subject": fold["held_out_subject"],
        "inner_val_subject": fold.get("inner_val_subject"),
        "train_subjects": list(fold["train_subjects"]),
        "training_split_mode": "all_non_held_out_no_inner_val",
        ...
        "learning_rate": float(learning_rate),
        "weight_decay": float(weight_decay),
        "torque_weight_knee_adduction_mode": str(fold_config.get("torque_weight_knee_adduction_mode", "absolute")),
        "requested_torque_weight_knee_adduction": float(
            fold_config.get(
                "requested_torque_weight_knee_adduction",
                fold_config.get("torque_weight_knee_adduction", 0.0),
            )
        ),
        "effective_torque_weight_knee_adduction": float(
            fold_config.get("effective_torque_weight_knee_adduction", loss_weights.get("torque_knee_adduction", 0.0))
        ),
        "torque_weight_knee_adduction_calibration": fold_config.get("torque_weight_knee_adduction_calibration"),
    }
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="Override left-stance knee adduction moment loss weight",
    )
    parser.add_argument(
        "--Torque_weight_knee_adduction_mode",
        "--torque_weight_knee_adduction_mode",
        ...
        default=None,
        help=(
            "Interpret --Torque_weight_knee_adduction as an absolute multiplier or as "
            "the target first-batch scaled KAM/scaled torque-loss ratio."
        ),
    )
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "during LOSO training and evaluation. Disable after reprocessing with the "
            "corrected ProcessData.py."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default=False,
        help=(
            "Deprecated / no-op: trunk-sway (subject*_TS) trials are now included by DEFAULT. "
            "Pass --DoNotIncludeTrunkSway to use only the normal trials instead."
        ),
    )
    parser.add_argument(
        "--DoNotIncludeTrunkSway",
        action="store_true",
        default=False,
        help=(
            "Exclude the subject*_TS trunk-sway trials from the LOSO folds so training and the "
            "held-out standard metrics use only the normal trials. Default: trunk-sway trials ARE "
            "included in each base subject's fold (grouped under the base subject)."
        ),
    )
    parser.add_argument(
        "--evaluateOnTS",
...
def main() -> None:
    ...
    if (
        _is_direct_torque_config(config)
        and inner_selection_objective == "selected_left_stance_moment_mae_percent_bwh_mean"
    ):
        inner_selection_objective = "direct_torque_mae_bwh"
        print(
            "Direct-torque checkpoint detected; using inner_selection_objective=direct_torque_mae_bwh.",
            flush=True,
        )
    ...
    # Trunk-sway trials are included by default; --DoNotIncludeTrunkSway opts out. The legacy
    # --includeTrunkSway flag is a no-op (already the default) kept for backward compatibility.
    include_trunk_sway = not bool(getattr(args, "DoNotIncludeTrunkSway", False))
    ...
    print(
        f"includeTrunkSway: {include_trunk_sway} "
        f"({'default (trunk-sway trials included)' if include_trunk_sway else '--DoNotIncludeTrunkSway set (normal trials only)'})",
        flush=True,
    )
    ...
    for result in fold_results:
        infer_summary = result.get("held_out_infer_style") or {}
    ...
    # Cohort-level trunk-sway KAM aggregation across all held-out subjects (only when
    # per-fold trunk-sway evaluation ran).
    trunk_sway_cohort_summary = None
    all_stance_kam_summary = None
    if evaluate_on_ts:
        trunk_sway_cohort_summary = _aggregate_trunk_sway_kam_across_folds(fold_results, output_root)
        all_stance_kam_summary = _aggregate_all_stance_kam_segments_across_folds(fold_results, output_root)
        if trunk_sway_cohort_summary is not None:
            _n = trunk_sway_cohort_summary.get("n_subjects", 0)
            _agree = trunk_sway_cohort_summary.get("agreement_LOSO_vs_GT", {})
            print(
                f"🦵 Cohort KAM trunk-sway summary written for {_n} subjects "
                f"(LOSO-vs-GT TS-effect r={_agree.get('mean_effect_pearson_r', float('nan')):.2f}, "
                f"MAE={_agree.get('mean_effect_mae', float('nan')):.3f} %BW*H) -> "
                f"{output_root / 'trunk_sway_cohort_summary'}",
                flush=True,
            )
        else:
            print("🦵 Cohort KAM trunk-sway summary skipped (no usable per-fold TS results).", flush=True)
        if all_stance_kam_summary is not None:
            _seg = all_stance_kam_summary.get("conditions", {})
            _normal_n = ((_seg.get("normal", {}) or {}).get("Ground Truth", {}) or {}).get("n_segments", 0)
            _ts_n = ((_seg.get("trunk_sway", {}) or {}).get("Ground Truth", {}) or {}).get("n_segments", 0)
            print(
                f"🦵 All-stance KAM condition plots written "
                f"(normal stances={_normal_n}, trunk-sway stances={_ts_n}) -> "
                f"{output_root / 'trunk_sway_cohort_summary'}",
                flush=True,
            )
        else:
            print("🦵 All-stance KAM condition plots skipped (no usable KAM trial payloads).", flush=True)
    ...
    summary_payload = {
        ...
        "trunk_sway_cohort_summary": trunk_sway_cohort_summary,
        "all_stance_kam_summary": all_stance_kam_summary,
        "training_excluded_subjects": training_excluded_subjects,
